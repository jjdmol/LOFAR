//#  tDH_SubBand.cc: Test DH_SubBand
//#
...
int main (int argc, char **argv) {
  ...
  try {
    DH_SubBand srcDH("subband", 1);
    DH_SubBand dstDH("subband", 1);
    srcDH.init();
    dstDH.init();
    TH_Mem TH;
    ...
    { // this is specific for DH_SubBand
      ...
      DH_SubBand::BufferType value = makei16complex(0,0);
      RectMatrix<DH_SubBand::BufferType>::cursorType beginCursor = srcMatrix.getCursor(0*stationDim + 0*freqDim + 0*polDim + 0*timeDim);
      RectMatrix<DH_SubBand::BufferType>::cursorType scursor, fcursor, tcursor, pcursor;

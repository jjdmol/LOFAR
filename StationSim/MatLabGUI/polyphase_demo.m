...
if strcmp(option,'fast') | strcmp(option,'fast-both');  
    K1=16; L1=15 ; M1=K1 ;   N2=16 ; K2=N2; L2=8  ; M2=K2 ;  nms=50000;     dt=10^(-6); 
    x=zeros(1,nms);  
    t=0:dt:dt*nms;    
    x=chirp(t,10,nms*dt,0.5/dt); 
    %x=sin(2*pi*t*nms*4)+ + sin(2*pi*t*nms*6) + 2^(-10) * randn(1,length(t));
...
if 1
  ...
  subplot(338); mesh(max(dBmin,20*log10(abs(X1)))) ; view(0,90) ; 
                     ylabel( 'FFT result') ; 
...
function polyphase_demo_plots(x,y,z,dBmin,row_nr,line_style)
  ...
  mesh( max(dBmin, 20*log10(abs(y)')) ) ; ylabel( 'Subband after one stage of filtering') ; view(0,90) ;               

  plot_nr=3+nr_columns*(row_nr-1) ;  subplot(nr_rows,nr_columns,plot_nr) ; 
  mesh( max(dBmin, 20*log10(abs(z)') ) ) ; 

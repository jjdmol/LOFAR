...
%       shift  -  if shift='up' the output is shifted to the centre by u sampling (default shift='none')
%                 if shift='down' the output is centered by down sampling
...
if nargin < 6
    ...
    h = fircls1(K-1, 0.95 * alpha, 0.01, 0.01);    
    % h= cheby1(K-1, 0.05, 0.999 ) ;
    % h= besself(K,0.9  ) ;
    % h = ellip(K,0.5,20,0.99) ;
    % h= butter(K,0.9) ;
    
end;

   h = resample(h, K * L, K) ; % version 1.0
   % h = resample(h, length(h) * floor(L * K/M), K) ; % version 1.1
   
   if strcmp(centre, 'up') ;
       h=exp(2*sqrt(-1)*pi*L/2*(1:1:length(h))/length(h)).*(K * L * h);
   else if strcmp(centre, 'down') ;
       h=exp(-2*sqrt(-1)*pi*L/2*(1:1:length(h))/length(h)).*(K * L * h);
   end;end;
...
if h_nq > 0
    ...
    h = double(uencode(h, h_nq, 1, 'signed'));
    h = h / max(h);                                 
end;

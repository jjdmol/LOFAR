function P = som_sammon(D, P, varargin)

%SOM_SAMMON Computes Sammon's projection for data or map.
%
% P = som_sammon(D, P, [value], [mode], [alpha], [Mdist])
%
% ARGUMENTS ([]'s are optional) 
%
%  D        (matrix or data struct or map struct) data to be projected
%            NOTE: the data may _not_ have unknown components
%  P        (scalar or matrix) output dimension, or the initial
%            projection matrix, size dlen x odim
...
%  [Mdist]  (matrix) mutual distances between data vectors in the input 
%            space, size dlen x dlen, by default the euclidian distances
%            are used
% 
% RETURNS   
%
...
%
% P = som_sammon(sMap,3)  
%  projects the codebook vectors of the map to 3-dimensions. The 
%  projection can be visualized with som_showgrid like this: 
%  som_showgrid(sMap,P)
%
% P = som_sammon(sMap,3,[],[],[],Md)
%  same as above, but uses a precalculated mutual distance matrix Md
%
% P = som_sammon(sMap,som_projection(sMap,3,'pca'),20,'steps')
%  same as above, but with PCA initialization, in which case the 
%  number of training steps can be lowered
%
% P = som_sammon(sData,2,60,'seconds')
%  projects the data matrix of sData to a plane iterating for 60 seconds
%
%
% See also SOM_PROJECTION, SOM_CCA, SOM_PCA, SOM_SHOWGRID, 
% SOM_DATA_STRUCT, SOM_CREATE.

% Copyright (c) 1997 by the SOM toolbox programming team.
% http://www.cis.hut.fi/projects/somtoolbox/

% Version 1.0beta juuso 260997, 090997, 191297, ecco 030997, 180997 
...
if nargin < 3 | isnan(varargin{1}) | isempty(varargin{1}), value=100;
else value = varargin{1}; end

if nargin < 4 | isnan(varargin{2}) | isempty(varargin{2}), mode='steps';
...
if isnan(alpha) | isempty(alpha), alpha = 0.2; end
...
if isempty(Mdist) | isnan(Mdist), 
  fprintf(2, 'computing mutual distances\r');
  for i = 1:noc,
    vec         = D(i, :);
    Diff        = D - vec(noc_x_1, 1:dim);
    Mdist(i, :) = sqrt(sum((Diff.^2)'));  
  end
else
  if size(Mdist) ~= [noc noc], 
...
while ~ready
  ...
  % you can speed the computation by about 20-25% by skipping this part
  ...
  % uncomment the code below; it is not in use by default to speed up 
  % computation.
  %
  %  clf
  %  if odim == 1, 

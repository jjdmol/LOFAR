...
bool Socket::openListener (const short		portnr)
{
	//# put the bytes in the right network-order
	short	portno = htons (portnr);
...
void Socket::setBuffer( int   buffersize, 
  ...
  if (bufferptr != NULL) {
    ...
    //std::cout << "accept: reuse buffer  " << std::endl;
    itsData = bufferptr;
    itsBufferSize = buffersize;
  } else {
    // no external puffer specified, so we will
    // allocate our own buffer
    //std::cout << "accept: create new buffer" << std::endl;
...
//#	Out:	--
//#	Return:	--
void Socket::allocBuffer (int	bufferSize)
{
	freeBuffer();
	itsIsAllocated=true;
		  ...
		  const int		messageLength)
{
	...
	//std::cout << "Socket::send  sent " << wLen << "/" << messageLength <<std::endl;
		  ...
		  const int		len)
{
  ...
  if (itsData == 0) {
    ...
    std::cout << "itsData buffer not initialised" << std::endl;
    return 0;
  }
  buf[0] = '\0';
  ...
  //LOG_TRACE_FLOW(formatString("Read on %d:length = %ld\n", itsPortnr, itsBufferSize));
   if (len && itsConnected) {
     ...
     if (!errno && (newBytes > 0)) {
       ...
       if (len != newBytes) std::cout << "did not read all data" << std::endl;
		       ...
		       const int		timeout)
{
	...
	int	presult = ::poll (fdset, 1, timeout);

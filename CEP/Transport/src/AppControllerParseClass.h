//# AppControllerParse.h: Class for parsing simulation commands
//#
//# Copyright (C) 2001
...
#ifndef CEPFRAME_APPCTLPARSE_H
#define CEPFRAME_APPCTLPARSE_H
...
/**
   AppControllerParse is the class for parsing simulation commands.
   ...
   See AppController_Example.cc for how this class can be used.
*/

class AppControllerParse
{
public:
  /// Parse and execute the commands to be read from standard input.
  static void parse (AppController&);
...
private:
  ...
  static AppController* theirAppController;
};



/**
   AppControllerParseError is used in case of parser errors.
   An exception with this class is thrown. The object contains
   the actual error message.

   One can put a try/catch block around AppControllerParse::parse to
   catch this error object and, for example, to output a message.
*/

class AppControllerParseError: public std::runtime_error
{
public:
  /// Construct the error object with the given message.
  AppControllerParseError (const string& message);
};

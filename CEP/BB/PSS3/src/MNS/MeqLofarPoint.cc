...
//   }
  ...
  for (int srcnr=0; srcnr<nrsrc; srcnr++) {
	...
	if (evalxx && evalyx) {
	  for (int i=0; i<ncell; i++) {
	    dcomplex sf11 = ps1*conj(r11[i]) + ps2*conj(r12[i]);
	    dcomplex sf21 = ps3*conj(r11[i]) + ps4*conj(r12[i]);
	...
	if (evalxy && evalyy) {
	  for (int i=0; i<ncell; i++) {
	    dcomplex sf12 = ps1*conj(r21[i]) + ps2*conj(r22[i]);
	    dcomplex sf22 = ps3*conj(r21[i]) + ps4*conj(r22[i]);

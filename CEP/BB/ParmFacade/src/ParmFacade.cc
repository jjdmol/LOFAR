            ...
            double starty, double endy, int ny)
{
    ...
    //}
    for(MeqParmGroup::iterator iter = parmGroup.begin(); iter != parmGroup.end(); ++iter)
    {
...
// Get the coefficients of parameters of type 'history' matching the parmNamePattern
// and the specified domain. This is a temporary hack to facillitate viewing the solve
                ...
                double startSolveTime, double endSolveTime)
{
    ...
    map<string, ParmValueSet> parmValues;
    itsPDB.getValues(parmValues, parmNamePattern, ParmDomain(startx, endx, starty, endy), -1);

    // The output is returned in a map.
    map<string, vector<double> > out;

    // Loop over all parameters. A parameter can have multiple ParmValues, one for each
    // timestamp. Parameters of type 'history' have exactly one coefficient. For each
    // parameter, create an array of coefficients (one coefficient per timestamp),
    // sorted on timestamp.
    for (map<string,ParmValueSet>::iterator it = parmValues.begin(); it != parmValues.end(); ++it)
    {
        vector<double> coeff;
        ...
        for (vector<ParmValue>::const_iterator value_it = values.begin(); value_it != values.end(); ++value_it)
        {
            // Only take ParmValue's of type 'history' with a timestamp within the specified time range.
            if(value_it->rep().itsType == "history" && 
               value_it->rep().itsTimeStamp >= startSolveTime &&
               value_it->rep().itsTimeStamp <= endSolveTime)
            {
                ASSERTSTR(value_it->rep().itsCoeff.size() == 1, "parms of type 'history' should have exactly one coefficient.");

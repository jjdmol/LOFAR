//# DummySolver.cc: VDM implementation of the meqcalibrater
//#
//# Copyright (C) 2003
  ...
  while( control().start(initrec) > 0)
  {
	...
	bool useSVD = params[UseSVD].as<bool>(false);
	  ...
	  solve(useSVD);
        ...
        {
          cdebug(2)<<"ENDSOLVE after "<<niter<<" iterations, converge="
		   <<converge<<endl;
...
void DummySolver::setSolvable (const vector<string>& parms,
...
void DummySolver::setPeel (const vector<int>& peelSourceNrs,
...
void DummySolver::solveSelect (const vector<int>& ant1,
...
void DummySolver::processHeader (const DataRecord& initrec,
...
void DummySolver::makeStatExpr (int ant, bool wsrtModel)
{
...
void DummySolver::makeBLExpr (bool wsrtModel)
{
  int blinx = 0;
...
void DummySolver::saveResiduals (const DataRecord::Ref& header)
{

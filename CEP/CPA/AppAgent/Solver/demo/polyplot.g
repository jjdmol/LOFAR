...
const plot_slice := function (ref pgp,ref slice,env=T,labels=F,ci=1)
{
  if( env )
  {
    x1 := as_float(min(slice.x));
    x2 := as_float(max(slice.x));
    y1 := as_float(min(slice.y));
    y2 := as_float(max(slice.y));
      ...
      verbose=2)
{
  ...
  # this is used to serialize the plot_param requests
  ...
  self.parent := frame(parent,side='top',title='Poly MEP viewer');
  self.frames := [=];
  self.frames.top := frame(self.parent,side='left',padx=0,pady=0,borderwidth=0);
  self.frames.btm := frame(self.parent,side='left',padx=0,pady=0,borderwidth=0);
  self.frames.f1  := frame(self.frames.top,side='top',padx=0,pady=0,borderwidth=0);
  self.frames.f2  := frame(self.frames.top,side='left',padx=0,pady=0,borderwidth=0);
  self.frames.f3  := frame(self.frames.btm,side='left',padx=0,pady=0,borderwidth=0);
  self.frames.f4  := frame(self.frames.btm,side='left',padx=0,pady=0,borderwidth=0);
  self.cmdpad   := frame(self.frames.f1,side='top',padx=0,pady=0,width=pgpsize,height=0);
  self.cmdframe := frame(self.frames.f1,side='top',width=pgpsize);
  ...
  if( is_boolean(ids) )
    ids := field_names(rec[1].poly);
  self.data_ids := ids;
  self.selected_id := ids[1];
  if( is_boolean(plotcolors) || len(plotcolors) != len(self.data_ids) )
    plotcolors := 1+(1:len(self.data_ids));
  self.plotcolors := [=];
  for( i in 1:len(self.data_ids) )
    self.plotcolors[self.data_ids[i]] := plotcolors[i];
  
  # work out a good width for the selector widgets
  width := max(strlen(field_names(rec)));
  if( len(self.data_ids) > 1 )
    width := max(width,strlen(self.data_ids));
  width := min(width,20);
  
  # build parameter selector
  self.paramframe := frame(self.cmdframe,side='left',expand='x',padx=0,pady=0);
#  self.paramlabel := label(self.paramframe,'Parameter: ',fill='none');
  self.paramselect := listbox(self.paramframe,
          width=width,height=min(6,len(rec)),fill='none');
  self.parampad := frame(self.paramframe,expand='x',width=10,height=5);
  self.paramselect->insert(field_names(rec));
  whenever self.paramselect->select do 
  {
    self.selected_param := $value+1;
    self.selected_id := self.data_ids[1];
    if( has_field(self,'idselect') )
    {
      self.idselect->select(0);
      self.idselect_fr->enable();
    }
  ...
  if( len(self.data_ids) > 1 )
  {
    self.idselect_fr := frame(self.cmdframe,side='left',padx=0,pady=0,expand='x');
    self.idselect_fr->disable();
    self.idselect := listbox(self.idselect_fr,
          width=width,height=min(3,len(self.data_ids)),fill='none');
    self.idselect_pad := frame(self.idselect_fr,expand='x',width=10,height=5);
    self.idselect->insert(self.data_ids);
    self.idselect->select(0);
    whenever self.idselect->select do
    {
      self.selected_id := self.data_ids[$value+1];
      if( has_field(self,'selected_param') )
        public.plot_param(self.selected_param,self.selected_id);
    }
  }
...
rec := mep2rec(demomep,field='original',range=T);
# add perturbed values, and override lousy time ranges

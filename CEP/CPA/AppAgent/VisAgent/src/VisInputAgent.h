...
//
// This is the vocabulary of events generated by VisAgents
//
namespace VisAgentVocabulary
{
  const HIID HeaderEvent      = AidHave|AidHeader;
  const HIID DataEvent        = AidHave|AidData;
  const HIID EndDataEvent     = AidEnd|AidData;
  const HIID InputErrorEvent  = AidInput|AidError;
};
...
{
  public:
      // Return codes for the getXXX()/hasXXX() methods
    //##ModelId=3DF9FECD016F
      typedef enum {
          SUCCESS   = 1,
          WAIT      = 0,    // must wait for data
          CLOSED    = -1,   // input stream is closed (or not connected)
          OUTOFSEQ  = -2,   // request is out of sequence (see below)
      } ReturnCodes;
         
      // Some standard events generated by input agents.
      // Note that some applications may choose to ignore these events 
      // completely and rely on return codes instead.
    //##ModelId=3DF9FECF029C
    //##Documentation
    //## generated when a data set header is available on the stream
      static const HIID & HeaderEvent  ()  { return VisAgentVocabulary::HeaderEvent; }
    //##ModelId=3DF9FECF02B7
    //##Documentation
    //## generated when data begins arriving on the stream      
       static const HIID & DataEvent    ()  { return VisAgentVocabulary::DataEvent; }
    //##ModelId=3DF9FECF02D1
    //##Documentation
    ...
    //## in combination with suspend()/resume() to control the data flow. 
      virtual int  numPendingTiles () const

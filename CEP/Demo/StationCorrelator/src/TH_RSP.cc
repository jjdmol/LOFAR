//# TH_RSP.cc: Transport mechanism for RAW Ethernet
//#
...
TH_RSP::TH_RSP(const string &ifname, 
               const string &rMac, 
               const string &oMac, 
               const uint16 etype, 
               const bool dhheader)
...
{
  LOG_TRACE_FLOW("TH_RSP constructor");
...
TH_RSP::~TH_RSP()
{
  LOG_TRACE_FLOW("TH_RSP destructor");
...
TH_RSP* TH_RSP::make() const
{
  return new TH_RSP(itsIfname, itsRemoteMac, itsOwnMac, itsEthertype, itsDHheader);
}
...
string TH_RSP::getType() const
{
  return "TH_RSP"; 
}

bool TH_RSP::recvBlocking(void* buf, int32 nbytes, int32 tag)
{  
  if (!itsInitDone) {
    return false;
  }
...
bool TH_RSP::recvVarBlocking(int32 tag)
{
...
bool TH_RSP::recvNonBlocking(void* buf, int32 nbytes, int32 tag)
{
  LOG_WARN( "TH_RSP::recvNonBlocking() is not implemented. recvBlocking() is used instead." );    
  return recvBlocking(buf, nbytes, tag);
}

bool TH_RSP::recvVarNonBlocking(int32 tag)
{
  LOG_WARN( "TH_RSP::recvVarNonBlocking() is not implemented. recvVarBlocking() is used instead." );    
  return recvVarBlocking(tag);
}

bool TH_RSP::waitForReceived(void*, int32, int32)
{
  LOG_TRACE_RTTI("TH_RSP waitForReceived()");
  return true;
}


bool TH_RSP::sendBlocking(void* buf, int32 nbytes, int32 tag)
{
  ...
  if (nbytes < 46) {
    LOG_ERROR_STR("TH_RSP::sendBlocking: payload must contain at least 46 bytes.");
    return false;
  }
...
bool TH_RSP::sendNonBlocking(void* buf, int32 nbytes, int32 tag)
{
  LOG_WARN( "TH_RSP::sendNonBlocking() is not implemented." );
  return false;
}

bool TH_RSP::waitForSent(void*, int32, int32)
{
  LOG_WARN( "TH_RSP::waitForSent() is not implemented." );
  return false;
}
...
{ 
  ...
  if (itsSocketFD < 0)
  {
    LOG_ERROR_STR("TH_RSP: socket could not be opened.");
    return;
  }
  ...
  if (ioctl(itsSocketFD, SIOCGIFMTU, &ifr) <0) {
    LOG_ERROR_STR("TH_RSP: getting the socket MTU failed.");
    close(itsSocketFD);
    return; 
  }
  else {
  ...
  {
    LOG_WARN("TH_RSP: send buffer size not increased, default size will be used.");
  }
  if (setsockopt(itsSocketFD, SOL_SOCKET, SO_SNDBUF, &val, sizeof(val)) < 0)
  {
    LOG_WARN("TH_RSP: receive buffer size not increased, default size will be used.");
  }
  ...
  if (itsOwnMac == "") {
    ...
    {
      LOG_ERROR_STR("TH_RSP: MAC address of ethernet device not found.");
      close(itsSocketFD);
      return;
    }  
  ...
  {
    LOG_ERROR_STR("TH_RSP: Packet filter failure.");
    close(itsSocketFD);
    return;
  }
  ...
  {
    LOG_ERROR_STR("TH_RSP: Interface index number not found.");
    close(itsSocketFD);
    return;
  }
  ...
  {
    LOG_ERROR_STR("TH_RSP: binding the socket to the interface failed.");
    close(itsSocketFD);
    return;
  }

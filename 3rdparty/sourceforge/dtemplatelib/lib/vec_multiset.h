...
public:
    ...
    iterator find(const Key& key)
	{
		...
		container_iterator begin = vec.begin();

		container_iterator lower = STD_::lower_bound(begin, vec.end(), key, pred);
		
		if (lower != vec.end() && !pred(key, *lower))
		{	
		   size_type elem_index =
				static_cast<size_type>(std_distance(begin, lower));
    ...
    const_iterator find(const Key& key) const
	{
		...
		// so use equal_range() and return end() if no elements found
		// otherwise return iterator to first element in that range
		container_const_iterator begin = vec.begin();

		container_const_iterator lower = STD_::lower_bound(begin, vec.end(), key, pred);
        ...
        equal_range(const Key& key) const
	{
		...
		size_type elem_index1 =
			static_cast<size_type>(std_distance(begin, result.first));
		size_type elem_index2 =
			static_cast<size_type>(std_distance(begin, result.second));

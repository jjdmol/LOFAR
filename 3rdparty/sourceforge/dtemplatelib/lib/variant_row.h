...
public:
	...
	STD_::vector<size_t>		  GetOffsets() const;
...
public:
	...
	template <class T> TypeTranslationField add_field(T *type) {

#if 1 // reentrant version
		if (p_data != NULL)
		{
			
			TypeTranslationField ttf(p_row_fields->types[tt_i], // type translation
		...
		//  throw VariantException(_TEXT("variant_row::add_field(T *type)"),
		//	tstring(_TEXT("Cannot add field to existing variant_row.")));
#endif // end reentrant version

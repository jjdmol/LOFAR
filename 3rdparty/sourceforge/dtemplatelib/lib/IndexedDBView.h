...
template<class DataObj, class BCA = DefaultBCA<DataObj> > class DynamicBCA
{
  ...
  public:
	  ...
	  DynamicBCA(const BoundIOs &boundIOs, const BCA &b, STD_::vector<tstring> orderOfParams) : dynbios(boundIOs),
		  bindMode(BoundIO::BIND_AS_PARAM), bca(b)
	  {
			  ...
			  for (size_t i = 0; i < orderOfParams.size(); i++)
			  {
				 ...
				 if (boundIO.IsColumn())
				 {
					...
					tstring numstring = numstr.str();

					// numstr.freeze(false);

					dynbios.insert(STD_::pair<const tstring, BoundIO>(numstring, boundIO));
				 }
...
#  endif
  ...
  protected:
	  ...
	  {
		 DataObj &data = const_cast<DataObj &>(dataObj);
		 BoundIOs boundIOs;
		 boundIOs.BindAsBase(data);
	
		 bca(boundIOs, data);

		 return (pDBview->GetInsVal())(boundIOs, data); 
	  ...
	  void init(const View &view, const tstring IndexNamesAndFields, KeyMode km) 
		{
			...
			TCHAR *semiColonToken = std_tstrtok(c_str, _TEXT(";"));
			...
			while (semiColonToken != NULL)
			{
				...
				semiColonToken = std_tstrtok(NULL, _TEXT(";"));
			}
			...
			while (tok_it != semiColonsParsedStrings.end())
			{
				...
				TCHAR *nameToken = std_tstrtok(reinterpret_cast<TCHAR *>(strTokMe.get()), _TEXT(" "));
				...
				if (std_tstricmp(nameToken, _TEXT("Unique")) == 0)
				{
					...
					nameToken = std_tstrtok(NULL, _TEXT(";"));
...
//             (uniqueness test succeeded or index is nonunique in the first place)
	   ...
	   bool should_check_uniqueness_on_fetch(HASH h)
	   {
		   return 1;
	   }
	   ...
	   template<class OutputIterator> 
		   void bulk_fetch_helper_aux(typename View::select_iterator &read_it,
		   size_t buffer_size, OutputIterator output, NO_GEN_BULK_FETCH)
	   {

...
template<class DataObj> STD_::pair<tstring, STD_::vector<tstring> >
	MakePostfixForUpdate(const BoundIOs &boundIOs, tstring QuoteChar, const DataObj *dummy)
{
	return PostfixForUpdate(boundIOs, _TEXT(""));
}

template<> inline STD_::pair<tstring, STD_::vector<tstring> >
	MakePostfixForUpdate(const BoundIOs &boundIOs, tstring QuoteChar, const variant_row *dummy)
{

...
private:
	   ...
	   BoundIOs ExtractPK(BoundIOs &IndexFields, STD_::vector<tstring> &KeyFields)
	   {
		   ...
		   for (STD_::vector<tstring>::iterator field_it = KeyFields.begin(); field_it != KeyFields.end(); field_it++)
		   {
	   ...
	   // extract the BoundIO's representing the columns that make up the autokey
	   BoundIOs ExtractAutoKey(BoundIOs &IndexFields)
	   {
		   // BoundIOs boundIOs(IndexFields);
		   BoundIOs boundIOs;
		   ...
		   STD_::vector<tstring> KeyFields = pview->GetKey();
...
public:
	   // exception safe!
	   void Init_bpabios(STD_::vector<tstring> &KeyFields) {
			...
			switch (useWhichFields)
			{
			     case USE_PK_FIELDS_ONLY: // get BoundIOs representing PK only
					 {
				       bpas = ExtractPK(bcabios, KeyFields);
					   break;
					 }
		...
		operator=(const DB_replacer<DataObj, ParamObj> &other)
	{
	   ...
	   // a new set of data as specified by dataObj 
 	   void replace(const DataObj &old_dataObj, const DataObj &dataObj)
	   {
		  ...
		  bcabios = bcabios.ChangedFields(old_dataObj, dataObj);
		  ...
		  if (postfix == _TEXT("")  && useWhichFields != USE_PK_FIELDS_ONLY)  {// use all fields or autokey if nothing set
			// useWhichFields = USE_ALL_FIELDS;
			STD_::vector<tstring> tmp_fields;
			Init_bpabios(tmp_fields);
		  }
		  ...
		  pNewView = new DBView<DataObj, DataObj>(MakeDelimitedList(pview->GetTableNames()),
				...
				DynamicBCA<DataObj, BCA>(bpabios, bca, orderOfColumns), pview->GetSelVal(), pview->GetInsVal(),
				monomorphize(pview->get_io_handler()),
				pview->GetConnection(), MakeDelimitedList(pview->GetKey()),
				useWhichFields);
		  ...
		  *up_it = dataObj; // InsValidate logic performed in proxy assignment
...
public:
		...
		operator=(const DataObj &data)
	{
		...
		if (sel_it != sel_it.GetView().end())
			replace(*sel_it, updated_dataObj);

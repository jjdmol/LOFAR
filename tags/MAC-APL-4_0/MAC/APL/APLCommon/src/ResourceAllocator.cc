...
INIT_TRACER_CONTEXT(ResourceAllocator,LOFARLOGGER_PACKAGE);
...
{
  LOG_TRACE_LIFETIME(TRACE_LEVEL_FLOW,"ResourceAllocator");
  if (0 == m_theInstance)
  {    
...
bool ResourceAllocator::_matchSOparameters(const TSOAllocation& current, const TSOAllocation& requested)
{
  return (current.samplingFrequency == requested.samplingFrequency);
}

bool ResourceAllocator::claimSRG(ResourceAllocator::LogicalDevicePtr ld, uint16 priority, 

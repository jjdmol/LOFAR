...
def rcu_freq(rcu_setting = None, subband = None):
    ...
    if (rcu_setting == None) or (subband == None):
        ...
        if ( (not rcu_setting["LBL_EN"] and not rcu_setting["LBH_EN"] and not rcu_setting["HB_EN"]) or
             (not rcu_setting["VH_EN"]  and not rcu_setting["VL_EN"]) ):
            # antenna  disabled
            return 'NA'
        else:
            #code here should be consistent with rcumode choice in sbstats_classifier_max_v2
            if (rcu_setting['BANDSEL'] == 1):
                # LB modes
                if (rcu_setting['LB_FILTER'] == 0):
                    # RCUMODE03 10-90 MHz
                    # NYQ zone 1, clk = 200MHz,
                    # spectral inversion?
                    chanwidth = 200./1024.
                    basefreq = 0.
                    freq = basefreq + (chanwidth * subband) + chanwidth/2.0
                else:
                    # RCUMODE04 30-90 MHz
                    chanwidth = 200./1024.
                    basefreq = 0.
                ...
                # BANDSEL=0 => HB modes
                if (rcu_setting['HB_SEL_0'] == 1):
                    if (rcu_setting['HB_SEL_1'] == 1):
                        # ALL OFF!
                        return('NA')
                    else:
                        # RCUMODE05 110-190MHz
                        # NYQ 2, clk=200
                        chanwidth = 200./1024.
                        basefreq = 100.
                        freq = 2 * basefreq - (chanwidth * subband) - chanwidth/2.0
                else:
                    if (rcu_setting['HB_SEL_1'] == 1):
                        # RCUMODE06 170-230 MHz
                        # NYQ 3, clk=160
                        chanwidth = 160./1024.
                        basefreq = 160.
                        freq = basefreq + (chanwidth * subband) + chanwidth/2.0
                    else:
                        # RCUMODE07 210-270 MHz
                        # NYQ 3, clk=200
                        chanwidth = 200./1024.
                        basefreq = 200.
                        freq = basefreq + (chanwidth * subband) + chanwidth/2.0
...
def classify(xst, rcu_byte = []):
    ...
    rcu_settings = rcu_settings_masks_v2.rcu_status(rcu_byte)
    onoff_rcus = rcu_settings_masks_v2.on_off_rcus(rcu_byte)
...
def array_geom(station_id = 1010, rcu_settings = [], epoch = datetime.datetime.now()):
    ...
    arraymode = rcu_settings_masks_v2.array_mode(rcu_settings)
    if (arraymode is None):
        arraymode = 3 # kludge force Low Band

    BANDSEL = -1
    
    if (arraymode == 3) or (arraymode == 4):
        BANDSEL = 1 # Low Band
    elif (arraymode == 5) or (arraymode == 6) or (arraymode == 7):
        BANDSEL = 0 # High Band

...
namespace LOFAR
{
  namespace CS1
  {
    ...
    //===============>>> MS_File::WriteDataPointFlags  <<<===============

    void   MS_File::WriteData(TableIterator* write_iter,
                              Matrix<Complex>* Data)
    {
      Table         WriteTable = (*write_iter).table();
      ArrayColumn  <Complex>     data(WriteTable, "DATA");
      data.put(0, (*Data));
    }
    //===============>>> MS_File  <<<===============
  }; // namespace CS1

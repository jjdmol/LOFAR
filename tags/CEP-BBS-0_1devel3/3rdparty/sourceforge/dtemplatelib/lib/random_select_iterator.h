...
private:
	...
	size_t row_size;
	enum {buffer_size=64}; // how many rows to fetch at a time
	...
	void fetch(SQLINTEGER row) {
		...
		// bulk read rows, do not perform SelValidate()
		read_it.bulk_copy(buf_begin, buf_end, false, row_size, base_row);
...
public:
	...
	SQLINTEGER get_lastrow() {
		...
		read_it.bulk_copy(buf_begin, buf_end, false, row_size, SelectIterator::BULK_END_POSITION);
		rows_in_buffer = read_it.GetLastCount();
		base_row = read_it.GetCount() - read_it.GetLastCount() + 1;
		last_row = read_it.GetCount();
	...
	const DataObj *get_row(SQLINTEGER row, bool &at_end)  {  
		if (base_row > 0 && row >= base_row && row < base_row + buffer_size) {
			...
			try {
				...
				SelValWrap<DataObj> &sel_val = read_it.GetSelValidate();
				BoundIOs bios(read_it.GetBoundIOs());
			...
			catch (RootException &ex1)
			{
				...
				error_action = read_it.get_io_handler()(ex1, read_it, *copy_me, read_it.Params());

				// if the handler tells us to throw 
				// then we will rethrow the original exception
				// for random access iterators - we do not allow a retry here since this messes up the iterator order

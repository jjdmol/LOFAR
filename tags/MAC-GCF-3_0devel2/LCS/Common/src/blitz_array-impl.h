 ...
 ***************************************************************************
 * $Log$
 * Revision 1.1  2002/11/05 12:44:21  smirnov
 * %[BugId: 107]%
 *
 * First version of array wrapper headers.
 * Note that blitz_array-impl.h should replace blitz/array-impl.h.
 *
...
#ifndef BZ_DEFAULT_ARRAY_STORAGE
  #define BZ_DEFAULT_ARRAY_STORAGE GeneralArrayStorage<N_rank>()
#endif
...
public:
    ...
    Array(GeneralArrayStorage<N_rank> storage = BZ_DEFAULT_ARRAY_STORAGE)
        : storage_(storage)
    {
        length_ = 0;
        stride_ = 0;
    ...
    _bz_explicit Array(int length0, 
        GeneralArrayStorage<N_rank> storage = BZ_DEFAULT_ARRAY_STORAGE)
        : storage_(storage)
    {
        length_[0] = length0;
        setupStorage(0);

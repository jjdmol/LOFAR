//#  OTDBinfo.h: For retrieving and modifying OTDB trees.
//#
...
#ifndef LOFAR_OTDB_OTDBINFO_H
#define LOFAR_OTDB_OTDBINFO_H
...
public:
	...
	OTDBinfo (OTDBconnection*	aConn,
		 	  treeIDType		aTreeID);
	// Construction by copying is allowed
	// OTDBinfo(const OTDBinfo&	that);	let compiler do the job
...
private:
	...
	OTDBinfo();
	OTDBinfo& operator=(const OTDBinfo& that);
...
//# --- Inline functions ---
inline bool	OTDBinfo::addKVT (const OTDBvalue&		aKVT)
{
	return (addKVT(aKVT.name, aKVT.value, aKVT.time));
}

inline string OTDBinfo::errorMsg() {
	return (itsError);
}

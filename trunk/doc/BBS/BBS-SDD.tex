...
%The \bbs kernel subsystem is the part responsible for all the number crunching.
%It is implemented as a stand-alone library that is used by the local controller
...
%The subsystem provides two main components: the \emph{kernel} (previously known
%as prediffer) and the \emph{solver}. The solver is discussed in separately in
%section~\ref{subsubsec:design-solver}.

%The kernel is a component with many responsibilities.
...
\todo{Check of bovenstaande paragraaf juist is.}

\subsubsection{Solver}
\label{subsubsec:design-solver}

The solver is responsible for estimating values for the parameters that minimise

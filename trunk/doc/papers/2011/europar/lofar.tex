...
\circlenumber{B} & Stokes I    & N & 16 & 24 & 327 &  74 Gb/s & 63 Gb/s & CPU   & Surveys \\
...
Figure \ref{fig:ionperf} shows the workload for the busiest I/O nodes in each case, including the time spent in the kernel to handle IRQs. Processing the station data and communicating with the compute cores cause most of the load. For cases \circlenumber{A} and \circlenumber{B}, the output is handled by less busy I/O nodes. Both cases form many beams, and thus require time to compute the positional weights that are required by the beam former (see Section \ref{Sec:pipeline_beamforming}). Cases \circlenumber{C}\circlenumber{D}\circlenumber{E}\circlenumber{F} do handle output, but otherwise show nearly identical behaviour.

...
second rack must be employed. We can exploit the compute power we saved to
create more beams, or to move additional computational steps, such as flagging, calibration and imaging,
...
The Cell BE is more energy efficient~\cite{Nieuwpoort:09}, but it does not incorporate a high-speed
interconnect.
...
% eigenlijk zouden we het feit dat er geen instructie voor loating point conversie eerst
% echt moeten uitleggen in het paper zelf. We moeten hier niet met nieuwe dingen op de proppen komen.
% dit geldt ook een beetje voor het stroomverbruik, etc.
...
Furthermore, We demonstrated that the correlator achieves extremely high performance,
...
resulting in a major improvement in the
effectiveness of the \emph{entire\/} telescope.

Traditionally, real-time telescope data are processed using customized hardware.
However, LOFAR's innovative, dishless design, with many thousands of omni-directional antennas, allows
new types of observations that need different processing pipelines.
The required flexibility is obtained by using the \emph{software} presented in this paper.
We are currently optimizing the pulsar pipelines that are functional, but not yet efficient enough.
Future work includes the integration of other processing pipelines,
and possibly real-time RFI removal, and real-time calibration.

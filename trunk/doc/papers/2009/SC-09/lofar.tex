...
we use the Blue Gene/P (BG/P) in an unconventional way: we run \emph{application\/}
...
The leftmost picture in Figure~\ref{lofar-overvew} shows the low-band
...
combined using analog beamforming. All antennas are designed for two
polarizations.  
...
in more detail in Section~\ref{sec:signal}.
...
The data loss due to missed UDP packets is low: only between 1 in $10^6$ and 1 in $10^4$ packets are dropped under
full load.

...
\emph{application\/} software on the so-called I/O~nodes to preprocessed and
...
while the roll-out of the real stations is currently in progress.
...
Unlike the Blue Gene/L, the torus uses DMA to offload the CPUs and allows
...
version. It successfully overlaps communication with useful work, reducing
...
limited connectivity (within a pset only), and their use of \emph{two\/}
...
application, and while many architectures (e.g., the IBM PowerXCell 8i, SSE)
provide twice the number of FLOPS for single-precision arithmetic, this is not
the case for the Blue Gene.
...
Nevertheless, we showed that we obtain extremely high performance,
both computationally and with respect to I/O.
...
necessary support to optimally use the BG/P.

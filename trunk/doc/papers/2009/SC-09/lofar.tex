...
\author{
\alignauthor John W. Romein\titlenote{To whom correspondence should be sent.}\\
...
\affaddr{Oude Hoogeveensedijk 4}\\
\affaddr{7991\ PD\ \ Dwingeloo}\\
...
combines signals from many thousands of simple antennas.
Its revolutionary design allows observations in a frequency range that has
hardly been studied before.

This paper focuses on another novel feature:
where traditional telescopes
use customized hardware, we process the data in \emph{software}. 
This dramatically increases flexibility and significantly reduces costs, but the high
processing and bandwidth requirements compel the use of a supercomputer.
The antenna signals are centrally combined, filtered, and
beamformed or correlated by a 10,880-core Blue Gene/P.

To meet the real-time requirements, the application is highly optimized, and
reaches exceptionally high computational and networking efficiencies.
The system performs so well that we can operate 50\% beyond the 
original LOFAR requirements, on less hardware, allowing
us to use the superfluous capacity for more accurate observations.
\end{abstract}
...
\hfill
\subfigure[For three observation modes.]{
  \hspace{15mm}
...
50\%, and only require a single Blue Gene/P rack to handle the input
from up to 64 stations. In order to meet the specifications, it was
necessary to use several parts of the Blue Gene/P in unconventional
ways, as well as work around weak points that we discovered during
development.
...
%We were able to push the performance of both the I/O nodes and the compute nodes to their extremes, and because
%of that we need only a single Blue Gene/P rack to process the data from up to 64 stations.

...
many-core architectures we use here, the bandwidth per operation is
3--10 times lower than on the BG/P, for instance.  Therefore, we must
...
The insight that the memory system is becoming more and more important
is not new.  The book by Catthoor et al.~\cite{data-access} is an
excellent starting point for more information on memory
...
processing, which has many similarities to multimedia.  Memory
optimization techniques can be subdivided in several classes. We can
make a distinction between hardware and software techniques. Next, we
can divide the software techniques into compiler optimizations and
...
software is not entirely black and white. A good example is the
...
Furthermore, the size of the cache lines can be taken into account. A
cache line is the smallest unit of memory than can be transferred
between the main memory and the cache.  Moreover, the associativity of
...
replaced.  Finally, prefetching can be used to load data into caches
or registers ahead of time.

Many cache-related optimization techniques have been described in the
literature, both in hardware and software. For instance,
hardware-based prefetching is described
in~\cite{Chen95effectivehardware-based}.  As we will describe in
Section~\ref{}, we implemented prefetching manually in the code, for
example by using multi-buffering on the Cell/B.E., or explicitly
...
the cache replacement policies work well. This well-known technique
is described, for instance, by Bacon et al.~\cite{cache-tlb-compiler}.
Many data access patterns optimization techniques are described
in~\cite{data-access}.

Many memory optimization techniques have been developed in the context
of optimizing compilers and runtime systems. For instance, much
research has been done on cache-aware memory allocation, see
e.g.,~\cite{Wilson95dynamicstorage}.  Compilers can apply many
techniques to optimize locality, by applying code and loop
transformations such as interchange, reversal, skewing and
tiling~\cite{data-locality}.  Also, compilers can optimize code for
...
\begin{figure}[t]
\begin{center}
%\includegraphics[width=0.8\columnwidth]{figures/performance-graph-v2.pdf} % for normal layout

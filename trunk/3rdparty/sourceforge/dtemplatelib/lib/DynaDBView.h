...
template<class DataObj> 
	STD_::vector<tstring> RemoveAppendedFieldsFromBCA(BCAWrap<DataObj> &bca, BoundIOs &boundIOs, 
...
template<typename DataObj> 
	   STD_::vector<tstring> GetAppendedFieldNames(BCAWrap<DataObj> &bca) {
...
public:
	...
	DynamicDBView(const tstring tableList, const tstring fieldList,
		...
		const KeyMode keyMode = USE_AUTO_KEY,

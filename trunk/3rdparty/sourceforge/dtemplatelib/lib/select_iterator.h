...
template <class DataObj> inline void SetWorkingRowAddr(DataObj *dummy, BoundIOs &b, BYTE *addr, TIMESTAMP_STRUCT *pts) {
	b.SetWorkingAddr(addr, pts);
}

template<> inline void SetWorkingRowAddr<variant_row>(variant_row *dummy, BoundIOs &b, BYTE *addr, TIMESTAMP_STRUCT *pts) {
	b.SetWorkingAddr(addr + sizeof(variant_row), pts);
}

template <class DataObj> inline void SetComplexRowAddr(DataObj *dummy, BoundIOs &b, BYTE *addr, TIMESTAMP_STRUCT *pts) {
	b.SetComplexAddr(addr, pts);
}

template<> inline void SetComplexRowAddr<variant_row>(variant_row *dummy, BoundIOs &b, BYTE *addr, TIMESTAMP_STRUCT *pts) {
				   ...
				   size_t row_size = sizeof(DataObj), SQLINTEGER position = BULK_DEFAULT_POSITION)
	{
		...
		if (pRowbuf.get() != pbegin) {
			if (!stmt.IsReady()) {
				...
				if (!(row_size >= sizeof(SQLINTEGER) * boundIOs.NumColumns()))
				{
					setstate(this->badbit);
				...
				pBytesFetchedArray = (SQLINTEGER *)boundIOs.pBytesFetchedArray.get() ;
				...
				stmt.SetStmtAttr(SQL_ATTR_ROW_ARRAY_SIZE, bufferRows, 0);
				stmt.SetStmtAttr(SQL_ATTR_ROW_BIND_TYPE, row_size, 0);
				...
				if (boundIOs.HasStrings()  || position != BULK_DEFAULT_POSITION) // if position != -1 require scrollable cursor
				{
				   ...
				   {
					 stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, SQL_CURSOR_STATIC, 0);
				   }
				   else 
				   {
					   ...
					   if (stmt.GetConnection().GetDBMSEnum() == DBConnection::DB_SQL_SERVER)
					   {
						 ...
						 stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, SQL_CURSOR_KEYSET_DRIVEN, 0);
					   }
					   else
					   {
						 stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, SQL_CURSOR_STATIC, 0);
				...
				boundIOs.pRowStatusArray.reset(bufferRows * sizeof(SQLUSMALLINT));
				boundIOs.pNumRowsFetched = new SQLUINTEGER(0);
				
				pRowStatusArray = (SQLUSMALLINT*)boundIOs.pRowStatusArray.get();
				
				// SetStmtAttr calls for row wise binding status flags
				stmt.SetStmtAttr(SQL_ATTR_ROW_STATUS_PTR, (SDWORD) pRowStatusArray, 0);
				stmt.SetStmtAttr(SQL_ATTR_ROWS_FETCHED_PTR, (SDWORD) boundIOs.pNumRowsFetched.get(), 0);
		...
		//
		BYTE *pBytesFetchedRow, *pBytesFetchedBackup;
		pBytesFetchedBackup = new BYTE[row_size];

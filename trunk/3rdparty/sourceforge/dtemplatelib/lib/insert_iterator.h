				   ...
				   size_t row_size = sizeof(DataObj))
	{
		...
		if (this->pRowbuf.get() != pbegin) {
			if (!this->stmt.IsReady()) {
				...
				if (!(row_size >= sizeof(SQLINTEGER) * this->boundIOs.NumColumns()))
				{
					setstate(this->badbit);
				...
				pBytesFetchedArray = (SQLINTEGER *)this->boundIOs.pBytesFetchedArray.get() ;
				...
				this->stmt.SetStmtAttr(SQL_ATTR_PARAM_BIND_TYPE, row_size, 0);
				this->stmt.SetStmtAttr(SQL_ATTR_PARAMSET_SIZE, bufferRows, 0);
				...
				this->boundIOs.pNumRowsFetched = new SQLUINTEGER(0);
				
				pRowStatusArray = (SQLUSMALLINT*)this->boundIOs.pRowStatusArray.get();
				
				// SetStmtAttr calls for row wise binding status flags
				this->stmt.SetStmtAttr(SQL_ATTR_PARAM_STATUS_PTR, (SDWORD) pRowStatusArray, 0);
				this->stmt.SetStmtAttr(SQL_ATTR_PARAMS_PROCESSED_PTR, (SDWORD) this->boundIOs.pNumRowsFetched.get(), 0);
				...
				this->stmt.OverrideStmtAttr(SQL_ATTR_PARAMSET_SIZE, 
					(pdata_end - pdata_begin) / row_size, 0);
		...
		SQLINTEGER numRows;
		numRows = (pdata_end - pdata_begin) / row_size;
		...
		try
		{
			...
			this->stmt.Execute();	
			SQLUINTEGER *pRow = this->boundIOs.pNumRowsFetched.get();

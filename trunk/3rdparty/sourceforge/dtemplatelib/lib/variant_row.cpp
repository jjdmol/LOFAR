...
variant_t::variant_t(void *p, const TypeTranslation &f) {
		...
		case C_USHORT: data = new Impl_t<unsigned short>(*((unsigned short *)p));break;
		case C_BOOL: data = new Impl_t<bool>(*((bool *)p));break;
		case C_INT: data = new Impl_t<int>(*((int *)p));break;
		...
		case C_WSTRING: data = new Impl_t<STD_::wstring>(*((STD_::wstring *)p));break;
#endif
...
// couldn't get template of the form template<T> operator T() to work w/o ambiguity.
	...
	STD_::wstring variant_t::get_wstring() const {
		...
		case C_STRING: {STD_::string *dmy=NULL; tostr << 
						   tstring_cast((STD_::wstring *)NULL, CastFromBase(data, dmy)->data); break;}
		...
		case C_STRING: {STD_::string *dmy=NULL; tostr << 
						   tstring_cast((STD_::wstring *)NULL, 
...
// ************* implementation code for variant_row_fields **********
	...
	STD_::vector<size_t>		  variant_row_fields::GetOffsets() const   { return offsets;	  }
...
variant_field::variant_field(const variant_t &v, variant_row *row, int field, const bool b) : m_val(v),
	...
	/*PS const*/ variant_field &variant_field::operator=(const NullField &null)
	{
		if (!IsNull())
			SetNull();
		return *this;
...
// ************ Implementation code for variant_row ************
	void variant_row::init_field(void *v, int typeId) {
	...
	void variant_row::copy_construct_field(void *dest, const void *src, int typeId) {
		...
		switch (typeId) {
		case C_STRING: {
				STD_::string *pstring = (STD_::string *)(src); 
				new(dest) STD_::string(*pstring);
				break;
				}

		case C_BLOB: {
				blob *pstring = (blob *)(src); 
				new(dest) blob(*pstring);
				break;
				}
#ifndef DTL_NO_UNICODE
		case C_WSTRING:{
				STD_::wstring *pwstring = (STD_::wstring *)(src); 
	...
	void variant_row::destroy_field(void *v, int typeId) {
		// destruct individual fields that are not primitives
		try {
			switch (typeId) {
			case C_STRING: {
			  using STD_::string;
			  STD_::string *pstring = (STD_::string *)v;
			...
			case C_WSTRING: {
			  using STD_::wstring;
			  STD_::wstring *pwstring = (STD_::wstring *)v;
#ifdef __hpux // LEAK!  XXX
			  //delete pwstring;
#else
			  pwstring->~wstring();
    ...
    char typeId = p_row_fields->types[field.m_col].typeId;
	  ...
	  // again g++ does not like these casts
		...
		case C_BOOL: *((bool *)p) = (bool)field.m_val; break;
	  ...
	  // need to use char* for the case below to work around g++ error
		...
		case C_BLOB: *((blob *)p) = field.m_val.get_blob(); break;
#ifndef DTL_NO_UNICODE
	...
	{	
		...
		STD_::vector<size_t> offsets = pRowFields1->GetOffsets();
		...
		for (size_t i = 0; i < vr1.size(); i++)
		{
			...
			// grab raw pointers
			void *pField1 = (void *) (((BYTE *) vr1.data_ptr()) + offset);
			void *pField2 = (void *) (((BYTE *) vr2.data_ptr()) + offset);

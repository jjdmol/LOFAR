function [SelectedSubBandSignals]=polyphase_antenna(AntennaSignals,K,L,M,option,SelectedSubBands)
number_antenna=size(AntennaSignals,1);
SelectedSubBandSignals=[];
for i=1:number_antenna
    x=AntennaSignals(i,:);
    [Yall] = polyphase_beam(x,K,L,M,option);
    SelectedSubBandSignals(i,:,:)=Yall(:,:);
end
dirpath='data';
load([dirpath '\output_options']);

if Polyphase_spectrum==1
figure(1)
surf(20*log10(abs(squeeze(SelectedSubBandSignals(1,:,:)))));
view(0,90);
shading interp;
title('Spectrum after polyphase filter of the first antenna');
end
% figure(23)
% plot(20*log10(abs(squeeze(SelectedSubBandSignals(1,:,SelectedSubBands)))));
% %TO DO when everything is working : 1 subplot per subband selected
% title(['Spectrum fo the selected subband : ' num2str(SelectedSubBands) ' after polyphase filter of the first antenna']);

save('data/antenna_signals.mat','AntennaSignals','SelectedSubBandSignals');




function [Yall,Yall_test] = polyphase(x,K,L,M,option,D)
%  function [Y] = polyphase(x,K,L,M,option,D)
%
...
%       D       - Experimental: discard D channels on either side prior to the second stage
%
...
shift='none' ; if strcmp(option,'centre') ; shift='up' ; end ;
   
% STAGE 1: Pre-filter by convolution with polyphase filter coefficients
ym=prefilter(x,K,L,M,shift); 

if D>0 ;  ym=ym(D+1:K-D,:) ; end ; % remove channels on the sides

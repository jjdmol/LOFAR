...
function [OutputSignal] = DFTFilterBankQuantised_gene2(InputSignal, quant_signal, quant_inputfft, quant_outputfft,...
...
for i = 1 : SubbandFilterLength
    for s = 1 : NumberOfSubbands
        h((i-1)*SubbandFilterLength+s)=SubFilter(s,i);
    end
end
...
for ob = 1: NumberOfBlocks-(SubbandFilterLength+1)
      for s = 1 : NumberOfSubbands % Normalization of the result of the convolution by 1/SubbandFilterLength to stay in [-1,1]
        ConvolutionOutput(s) = 1 / SubbandFilterLength * DownsampledSignal(s,:,ob) * SubFilter(s, :)'; 
        % Do only an addition
...
%
  ...
  if Stage > 1  
    if ob == 1
      %  OutputSignal = FFTOutput(NumberOfSubbands-overlap/2:-1:overlap/2);
         OutputSignal = FFTOutput(NumberOfSubbands:-1:1);
     ...
     %   OutputSignal = [OutputSignal; FFTOutput(NumberOfSubbands-overlap/2:-1:overlap/2)];
       OutputSignal = [OutputSignal; FFTOutput(NumberOfSubbands:-1:1)];

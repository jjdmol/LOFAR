//  WH_Station.h: An empty WorkHolder (doing nothing)
//
...
#ifndef BASESIM_WH_STATION_H
#define BASESIM_WH_STATION_H
...
class WH_Station: public WorkHolder
{
 public:
  ...
  WH_Station (const string& name, unsigned int nrcu);
  ...
  virtual WH_Station* make (const string& name) const;
...
private:
  ...
  WH_Station (const WH_Station&);

  /// Forbid assignment.
  WH_Station& operator= (const WH_Station&);

  DH_SampleC itsInDataHolder;

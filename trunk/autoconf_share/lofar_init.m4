...
[
  ...
  *_*)
  ...
  lfr_pkg=`basename $lfr_srcdir`;    # includes possible version extension
  \rm -f pkginc/$lfr_pkg;
  ...
  # Similarly files fillversionInc/Func.h are created to contain the body of the
  # fillVersion function. It is included by <package>Version.cc created
  # by the makeversion script.
  touch fillversionInc.old-h
  if [ -f fillversionInc.h ]; then
    mv fillversionInc.h fillversionInc.old-h
  fi
  # Create the fillversionInc.h file.
  \rm -f fillversionInc.h*
  echo "/* Generated by lofar_init.m4 */" >> fillversionInc.h
  echo "" >> fillversionInc.h
  # If the current fillversionInc.h is the same as the old one, copy the
  # old one back, while preserving creation date and time.
  ...
  touch fillversionFunc.old-h
  if [ -f fillversionFunc.h ]; then
    mv fillversionFunc.h fillversionFunc.old-h
  fi
  # Create the fillversionFunc.h file.
  \rm -f fillversionFunc.h*
  echo "/* Generated by lofar_init.m4 */" >> fillversionFunc.h-pkg
  echo "" >> fillversionFunc.h-pkg
  echo "namespace LOFAR {" >> fillversionFunc.h-pkg
  echo "  void ${PACKAGE}Version::fillVersion (std::vector<std::string>& v)" >> fillversionFunc.h-pkg
  echo '  {' >> fillversionFunc.h-pkg
  echo '    // Add version of this package.' >> fillversionFunc.h-pkg
  echo "    Version::fillVersion (\"${PACKAGE}\", getVersion(), v);" >> fillversionFunc.h-pkg
  echo '    // Add version of packages being used.' >> fillversionFunc.h-pkg

#!/bin/bash
#
# /StorageAppl: a start/stop/status script for swlevel
#
# Copyright (C) 2007
...
#
# Syntax: StorageAppl start|stop|status
#
# $Id$
#
...
stop_prog()
{
	...
	cexec killall -9 orted CS1_Storage
	killall -9 mpirun 
}
...
status_prog()
{
	...
	cexec 'ps -ef | grep -v grep '| grep CS1_Storage| grep -v bash 2>$1 1>/dev/null
	if [ $? -eq 0 ]; then
	  cexec 'ps -ef | grep -v grep | grep CS1_Storage'| grep -v bash | awk -v levelnr=${levelnr} '{
          if (substr($1,1,3) == "---") {
            machine = substr($2,1,7)
          }
          else {
            if (substr($1,1,3) != "***") {
              fullname="StorageAppl@"machine
              printf "%s : %-25.25s %s\n", levelnr,fullname,$2
            }
          }
        }'

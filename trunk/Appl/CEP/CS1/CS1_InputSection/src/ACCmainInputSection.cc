...
namespace LOFAR {
  namespace CS1 {
      ...
      int ACCmainInputSection (int argc, char* orig_argv[], ACC::PLC::ProcessControl* theProcess) {
	...
	try {
	  ...
	  if ((argc!=3) || (strncmp("ACC", argv[1], 3) != 0)) {
		...
		ParameterSet ParamSet = ps->makeSubset(prefix);
		 ACC::APS::globalParameterSet()->adoptCollection(ParamSet);
	      } catch (APSException&) {
		 ACC::APS::globalParameterSet()->adoptCollection(*ps);
	      }
	      delete ps;
	    }
	    ...
	    ParameterSet ParamSet(argv[2]);
	    string procID = ParamSet.getString("_processName");
	    string prefix = ParamSet.getString("_parsetPrefix");

	    ACC::APS::globalParameterSet()->adoptCollection(ParamSet);		       

	    ACC::PLC::ProcControlServer pcServer(ParamSet.getString(prefix + "_ACnode"),
				                 ParamSet.getUint16(prefix + "_ACport"),
				                 theProcess);

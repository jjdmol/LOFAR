...
\begin{document}
  ...
  \label{sec:blackboard-architectural-pattern}
    ...
    they can in order to approach an optimal quality of a solution,
    and put the results back to that central place.
  ...
  \label{sec:LOFAR-requirements}\hypertarget{sec:LOFAR-requirements}{}
    ...
    specified for this in order to make extending and tweaking
    somewhat error proof.

    A knowledge source's result definition must include an
    interpreter object that can return a quality of the result. For a
    controller it must be clear to what kind of new workload the
    result leads.
    ...
    avoided. Hence the knowledge source should be started and ready,
    or the assignment should be queued.
    ...
    or priority. As mentioned above this gives a controller the
    freedom to act as and when it deems fit. This is also useful if
    the current workload is not the only possible cause for a new
    workload for a certain engine.
  ...
  \end{figure}
    ...
    \label{subsec:pattern-components}
      ...
      \begin{figure}
        \includegraphics[]{../figures/selfcal-overview.eps}
...
%%      \paragraph{parameter \#3}
  ...
  \label{sec:lessons-learned}
    ...
    watchers hang around to compare and evaluate intermediate results, have no or
...
% LocalWords:  ASTRON selfcal BlackBoard

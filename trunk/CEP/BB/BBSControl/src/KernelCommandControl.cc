...
void KernelCommandControl::handle(BBSStrategy &command)
{
    LOG_DEBUG("Handling a BBSStrategy");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSStep &command)
{
    LOG_DEBUG("Handling a BBSStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSMultiStep &command)
{
    LOG_DEBUG("Handling a BBSMultiStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSSingleStep &command)
{
    LOG_DEBUG("Handling a BBSSingleStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSPredictStep &command)
{
    LOG_DEBUG("Handling a BBSPredictStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSSubtractStep &command)
{
    LOG_DEBUG("Handling a BBSSubtractStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSCorrectStep &command)
{
    LOG_DEBUG("Handling a BBSCorrectStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSSolveStep &command)
{
    LOG_DEBUG("Handling a BBSSolveStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSShiftStep &command)
{
    LOG_DEBUG("Handling a BBSShiftStep");
    LOG_DEBUG_STR("Command: " << endl << command);
}

void KernelCommandControl::handle(BBSRefitStep &command)
{
...
strategy->correlation());
    ...
    bool KernelProcessControl::handle(const BBSStep *bs)
    {
        ...
        bool result = false;
        {
            const BBSPredictStep* step = dynamic_cast<const
BBSPredictStep*>(bs);
            if(step)
            {
                result = handle(step);
        ...
        {
            const BBSSubtractStep* step = dynamic_cast<const
BBSSubtractStep*>(bs);
            if(step)
            {
                result = handle(step);
        ...
        {
            const BBSCorrectStep* step = dynamic_cast<const
BBSCorrectStep*>(bs);
            if(step)
            {
                result = handle(step);
...
rectangular grid,
        // with cells of size step->itsDomainSize. Should become more
...
dynamic_cast<BlobStreamableVector<IterationResult>*>(itsSolverConnection->
recvObject());
            ASSERT(resultv);
...
context.solveDomains.size()) / 100.0);
            finished = (converged == context.solveDomains.size()) || (iteration
== step->maxIter());
            iteration++;
        }

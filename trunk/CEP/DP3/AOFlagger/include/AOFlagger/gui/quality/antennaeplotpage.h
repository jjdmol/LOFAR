	...
	protected:
		virtual void processStatistics(class StatisticsCollection *statCollection, const std::string &filename)
		{
			...
			for(vector<std::pair<unsigned, unsigned> >::const_iterator i=baselines.begin();i!=baselines.end();++i)
			{
				if(i->first != i->second)
				{
					_statistics.insert(std::pair<double, Statistics>(i->first, map.GetStatistics(i->first, i->second)));
					_statistics.insert(std::pair<double, Statistics>(i->second, map.GetStatistics(i->first, i->second)));
				}
			}
		}

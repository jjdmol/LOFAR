...
#include <AOFlagger/gui/quality/timeplotpage.h>

#include <AOFlagger/quality/statisticscollection.h>
#include <AOFlagger/quality/statisticsderivator.h>

TimePlotPage::TimePlotPage() :
...
template<enum TimePlotPage::PhaseType Phase>
double TimePlotPage::getValue(const std::complex<float> val)
{
	switch(Phase)
	{
...
template<enum TimePlotPage::PhaseType Phase>
void TimePlotPage::plotPhase(QualityTablesFormatter::StatisticKind kind, unsigned polarization)
{
	std::ostringstream s;
	s << "Polarization " << polarization;
	_plot.StartLine(s.str(), "Time", "Value (Jy)", true);
	StatisticsDerivator derivator(*_statCollection);
	const std::map<double, Statistics> &statistics = _statCollection->TimeStatistics();
	for(std::map<double, Statistics>::const_iterator i=statistics.begin();i!=statistics.end();++i)
	{
		...
		_plot.PushDataPoint(time, getValue<Phase>(val));
	}
}

template<enum TimePlotPage::PhaseType Phase>
void TimePlotPage::plotPhase(QualityTablesFormatter::StatisticKind kind, unsigned polarizationA, unsigned polarizationB)
{
	std::ostringstream s;
...
void TimePlotPage::plotStatistic(QualityTablesFormatter::StatisticKind kind)
{
...
void TimePlotPage::plotPolarization(QualityTablesFormatter::StatisticKind kind, unsigned polarization)
{
...
void TimePlotPage::plotPolarization(QualityTablesFormatter::StatisticKind kind, unsigned polarizationA, unsigned polarizationB)
{
	if(_amplitudeButton.get_active())
...
void TimePlotPage::initStatisticKindButtons()
{
	_countButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_statisticBox.pack_start(_countButton, Gtk::PACK_SHRINK);
	
	_meanButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_statisticBox.pack_start(_meanButton, Gtk::PACK_SHRINK);
	
	_varianceButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_varianceButton.set_active(true);
	_statisticBox.pack_start(_varianceButton, Gtk::PACK_SHRINK);
	
	_dCountButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_statisticBox.pack_start(_dCountButton, Gtk::PACK_SHRINK);
	
	_dMeanButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_statisticBox.pack_start(_dMeanButton, Gtk::PACK_SHRINK);
	
	_dVarianceButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_statisticBox.pack_start(_dVarianceButton, Gtk::PACK_SHRINK);
	
	_rfiRatioButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
...
void TimePlotPage::initPolarizationButtons()
{
	_polXXButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_polarizationBox.pack_start(_polXXButton, Gtk::PACK_SHRINK);
	
	_polXYButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_polarizationBox.pack_start(_polXYButton, Gtk::PACK_SHRINK);
	
	_polYXButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_polarizationBox.pack_start(_polYXButton, Gtk::PACK_SHRINK);
	
	_polYYButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_polarizationBox.pack_start(_polYYButton, Gtk::PACK_SHRINK);
	
	_polXXandYYButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_polXXandYYButton.set_active(true);
	_polarizationBox.pack_start(_polXXandYYButton, Gtk::PACK_SHRINK);
	
	_polXYandYXButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
...
void TimePlotPage::initPhaseButtons()
{
	_amplitudeButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_amplitudeButton.set_active(true);
	_phaseBox.pack_start(_amplitudeButton, Gtk::PACK_SHRINK);
	
	_phaseButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_phaseBox.pack_start(_phaseButton, Gtk::PACK_SHRINK);
	
	_realButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));
	_phaseBox.pack_start(_realButton, Gtk::PACK_SHRINK);
	
	_imaginaryButton.signal_clicked().connect(sigc::mem_fun(*this, &TimePlotPage::updatePlot));

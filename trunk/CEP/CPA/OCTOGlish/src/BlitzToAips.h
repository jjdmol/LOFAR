...
void copyArray ( Array<T> &out,const blitz::Array<T,N> &in )
{
  T *data = new T[in.size()],*ptr = data;
  for( typename blitz::Array<T,N>::const_iterator iter = in.begin();
      iter != in.end(); iter++,ptr++ )
    *ptr = *iter;
  out.takeStorage(convertShape(in.shape()),data,TAKE_OVER);
}
...
template<class T,int N>
void refArray ( Array<T> &out,blitz::Array<T,N> &in )
{
  FailWhen( in.ndim() != N,"array rank mismatch" );
  if( !in.isStorageContiguous() )
    copyArray(out,in);
  else
    out.takeStorage(convertShape(in.shape()),in.data(),SHARE);
}
...
template<class T,int N>
void aipsToBlitz ( blitz::Array<T,N> &out,Array<T> &in,int policy )
{
  FailWhen( in.ndim() != N,"array rank mismatch" );
  ...
  blitz::Array<T,N> tmp(data,shape,

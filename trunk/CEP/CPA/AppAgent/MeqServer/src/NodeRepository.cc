...
InitDebugContext(NodeRepository,"NodeRepository");
...
int NodeRepository::create (DataRecord::Ref::Xfer &initrec)
{
  string classname;
  Node::Ref noderef;
...
int NodeRepository::remove (int node_index)
{
...
Node & NodeRepository::get (int node_index)
{
  FailWhen(node_index<=0 || node_index>int(nodes.size()),
          "invalid node index");
  Node::Ref &ref = nodes[node_index];
...
bool NodeRepository::valid (int node_index) const
{
...
int NodeRepository::findIndex (const string& name) const
{
...
Node & NodeRepository::findNode (const string &name)
{

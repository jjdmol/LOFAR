 ...
 *** NB:  if skyModel option is used VLSSuse parameter must be not set. ***
               * Case 1: VLSSuse=yes => GSM sky model (VLSS) is used for inital calibration
               * Case 2: VLSSuse=no => low resolution (90 arcsec) skymodel is used for inital calibration (generated with from provided data)
               * Case 3: Skymodel is provided (VLSS used  is not provided), your own skymodel is used for inital calibration
...
*  --VLSSuse(string,default:yes): yes or no => Use the VLSS Skymodel for the first iteration of selfcal (generated by gsm.py).
...
*  --maskDilation(integer,default:0): If mask is used (generated from pybdsm extracted skyModel), it is possible to dilate it. 0 => no dilatation, 1 => extend the no dilated mask by 1 pixel in all directions, etc ...
...
if __name__=='__main__':
    ...
    if  int(maskDilation) < 0: 
		print ''
		print 'maskDilation parameter must be equal or greater to 0 !' 
 		print ""
		sys.exit(2)
    ...
    try:
	      ...
	      maskDilation = int(maskDilation)     
    except:
	      print ""
	      print "maskDilation parameter must be an integer or not provided !"
	      print ""
	      sys.exit(2)

...
{ LOG_INFO("floatCompare");                                                \
  ...
  ASSERT(Numeric::compare(zero, negativeZero, 0));                         \
                                                                           \
  /* Make sure that nearby numbers compare as equal.                    */ \
  ASSERT(Numeric::compare(2.0f, 1.9999999f));                              \
                                                                           \
  /* Make sure that slightly more distant numbers compare as equal.     */ \
  ASSERT(Numeric::compare(2.0f, 1.9999995f));                              \
                                                                           \
  /* Make sure the results are the same with parameters reversed.       */ \
  ASSERT(Numeric::compare(1.9999995f, 2.0f));                              \
                                                                           \
  /* Make sure that even more distant numbers don't compare as equal.   */ \
  ASSERT(!Numeric::compare(2.0f, 1.999995f));                              \
                                                                           \
  /* The next set of tests check things where the correct answer isn't  */ \
  /* as obvious or important. Some of these tests check for cases that  */ \
  /* are rare or can easily be avoided. Some of them check for cases    */ \
  ...
  /* than the behavior of the fussier Final function.                   */ \
                                                                           \
  /* Test wrapping from inf to -inf when maxUlps is large.              */ \
  ASSERT(!Numeric::compare(inf, -inf, 16*1024*1024));                      \
                                                                           \
  /* Test whether FLT_MAX and infinity (representationally adjacent)    */ \
  /* compare as unequal.                                                */ \
  ASSERT(!Numeric::compare(FLT_MAX, inf));                                 \
                                                                           \
  /* Test whether a NAN compares as unequal to itself.                  */ \
  ASSERT(!Numeric::compare(nan, nan));                                     \
                                                                           \
...
#define doubleCompare                                                       \
{ LOG_INFO("doubleCompare");                                                \
                                                                           \
  /* The first set of tests check things that any self-respecting       */ \
  /* comparison function should agree upon.                             */ \
                                                                           \
  ...
  ASSERT(Numeric::compare(zero, negativeZero, 0));                         \
                                                                           \
  /* Make sure that nearby numbers compare as equal.                    */ \
  ASSERT(Numeric::compare(2.0, 1.999999999999995));                        \
                                                                           \
  /* Make sure that slightly more distant numbers compare as equal.     */ \
  ASSERT(Numeric::compare(2.0, 1.99999999999999));                         \
                                                                           \
  /* Make sure the results are the same with parameters reversed.       */ \
  ASSERT(Numeric::compare(1.99999999999999, 2.0));                         \
                                                                           \
  /* Make sure that even more distant numbers don't compare as equal.   */ \
  ASSERT(!Numeric::compare(2.0, 1.99999999999995));                        \
                                                                           \
  /* The next set of tests check things where the correct answer isn't  */ \
  /* as obvious or important. Some of these tests check for cases that  */ \
  /* are rare or can easily be avoided. Some of them check for cases    */ \
  ...
  /* than the behavior of the fussier Final function.                   */ \
                                                                           \
  /* Test wrapping from inf to -inf when maxUlps is large.              */ \
  ASSERT(!Numeric::compare(inf, -inf, (long long)1 << 52));                \
                                                                           \
  /* Test whether FLT_MAX and infinity (representationally adjacent)    */ \
  /* compare as unequal.                                                */ \
  ASSERT(!Numeric::compare(DBL_MAX, inf));                                 \
                                                                           \
  /* Test whether a NAN compares as unequal to itself.                  */ \
  ASSERT(!Numeric::compare(nan, nan));                                     \
                                                                           \

...
#ifndef LOFAR_MATH_LORRAYS_BLITZ_H
#define LOFAR_MATH_LORRAYS_BLITZ_H
...
namespace LOFAR 
{
  ...
  {  
    ...
    // 
    class VariVector : public vector<int>
    {
    public:
      ...
      explicit VariVector (int n1) 
        : vector<int>((n1&SETRANK)?(n1&~SETRANK):1,(n1&SETRANK)?0:n1) {};
      // can be copied from simple vector, too
      VariVector (const vector<int> &other)
        : vector<int>(other) {}; 
      // constructors for 2..5 dimensions
      VariVector (int n1,int n2) : vector<int>(2)
      { iterator iter = begin(); *iter++=n1; *iter++=n2; }
      VariVector (int n1,int n2,int n3) : vector<int>(3)
      { iterator iter = begin(); *iter++=n1; *iter++=n2; *iter++=n3; }
      VariVector (int n1,int n2,int n3,int n4) : vector<int>(4)
      { iterator iter = begin(); *iter++=n1; *iter++=n2; *iter++=n3; *iter++=n4; }
      VariVector (int n1,int n2,int n3,int n4,int n5) : vector<int>(5)
      ...
      VariVector( const blitz::TinyVector<int,N> &tvec )
        : vector<int>(tvec.data(),tvec.data() + N) {};
      ...
      VariVector(const casa::IPosition &ipos)
        : vector<int>(ipos.storage(),ipos.storage()+ipos.nelements()) {};
    ...
    inline ostream & operator << ( ostream &str,const VariVector &vec )
    {

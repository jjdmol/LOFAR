...
This paper describes the LOFAR signal processing chain from the antennas in the field to the central processing. The central processing is split in real-time processing and offline calibration and imaging.
\end{abstract}
...
\fixme{Andre: introduceer het begrip "central core" + een mooie aansluiting
...
Since the optimized bandwidth of the operating frequency range spans 5~octaves,
...
\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{Nyquistzonesmodes.eps}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{phase_error.eps}
\end{center}
...
in assembly.
\fixme{even naar kijken}
The FIR filters implicitly convert the 16+16-bit complex integer numbers from
the stations to 32+32-bit
...
\fixme{twee bijzinnen; gebruik van "we"}
An example that illustrates the benefits of a flexible software solution, was
the ease with which the second-stage PPF, which was originally designed to
run on FPGAs at the stations, could be moved to the BG/L.
...
a considerable cost reduction was achieved by removing the second-stage PPF
from the station design.
...
In the course of this year 18 full stations will be manufactured and installed in the field. Also the WAN infrastructure and central processor facility will be scaled up to handle the data of the 18 full stations and the international stations.

The remainder of the stations will be built in 2009. Currently the software development is focused on the imaging mode. In the course of this year significant effort will be put in realizing the tied-array beamforming mode.

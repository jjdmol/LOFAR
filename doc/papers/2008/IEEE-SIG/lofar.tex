...
Second, LOFAR's large FoV makes it uniquely suited to study {\em transient
sources}.
...
Despite the new concept the processing steps of a radio telescope remain the same. The general data path for an aperture synthesis array is depicted in Figure~\ref{fig:concept}.
...
The low band array will be a randomized and exponentially space tapered configuration~\cite{capp:06} with a size of about 85~m in diameter, while the HBA array will be installed as a dense and more regular array. 
...
\begin{figure}
\begin{center}
\includegraphics[width=93mm]{Nyquistzonesmodes.eps}
\end{center}
...
LOFAR uses a 12,288-core IBM Blue Gene/L supercomputer for the real-time
data processing,
...
Next in the pipeline is the second-stage PolyPhase Filter (PPF), that splits
...
\fixme{only standard imaging mode implemented}

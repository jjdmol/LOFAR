...
The beamformer can be implemented by using true time delays or by applying phase shifts on narrow subbands. For the last solution an error is made at the edges of each subband (Figure 7), since the phase is frequency dependent and only one phase can be set per subband. 
...
Since UDP is an unreliable protocol, it handles duplicated, lost, and
...
Each input node receives data from up to 54~subbands from a single station.
...
(resp.\ 610~Hz) wide.
The PPF filter consists of 256~Finite Impulse Filters (FIR) filters, of which
the outputs are fourier transformed.
Each FIR filter is a 16-taps band pass filter.

...
from the LOFAR stations in the field to the Blue Gene/P, the internal
data transpose, and finally to the storage cluster. In contrast, in~\cite{Romein:06}
...
first time, we perform a scalability analysis, proving that we can in
fact support the requirements of the full LOFAR instrument, both in
terms of computing and I/O, in real time.
...
allows application code to be run on the I/O~nodes.  The standard BG/P
...
mode.
We chose the BG/P as the central processing platform,
instead of, for instance, a cluster with a fast local interconnect, for
several reasons.  First, the BG/P is a power efficient supercomputer.
...
efficient supercomputers (see www.green500.org). In addition, the BG/P has excellent hardware
...
relative memory bandwidth. This leads to superior performance for our
data intensive applications, compared to other platforms~\cite{Nieuwpoort:09}.
Finally, the BG/P provides a high-speed 3D-torus network that can
...
data exchange.
Since the samples have an accuracy of at most 16-bit, single precision floating
point is enough for our purposes. Future telescopes, such as
the SKA, will likely use even less bits per sample.

...
%% 2) BG/P: ingebouwde supprot voor complexe operaties
...
\end{figure*}
%The pipeline runs from left to right. @@@ maak pijlen in fig
...
quadratically with the number of receivers. All other steps have a
...
particular time. Single precision floating point is accurate enough
for our purposes.  The receivers are dual-polarized; they take
separate samples from orthogonal (X and Y) directions.
% meer zeggen over precisie? 4, 8, en 16 bit modes voor LOFAR, SKA zelfs 2 bit.
...
themselves. 
The correlator thus is mostly multiplying and adding complex numbers.
...
operations. To perform these operations, we have to load the samples two receivers from memory.
The samples each consist of
four single precision floating point numbers: a real and imaginary part, and two polarizations.

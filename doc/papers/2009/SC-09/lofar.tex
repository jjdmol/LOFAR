...
A load above 85\% must be avoided to prevent data loss.

The left part of the graph shows the performance breakdown for one of the
most challenging modes: 64~stations, 8-bit samples, and up to 496~subbands
(the maximum station data rate).
The subbands are more or less evenly divided over the 64~psets, thus each
pset has to process up to 8~subbands.
In this case, most I/O~nodes receive and forward the samples of one station
(up to 3.1~Gb/s) and send the correlations of up to 8~subbands to storage
(up to 1.2~Gb/s).
...
The right part of the graph shows the performance breakdown for two other
challenging modes as well.
...
%\begin{figure}
%\includegraphics[width=\columnwidth]{cn-perf-8bit-station.pdf}
...
\caption{Compute node performance breakdown, as function of number of stations.}
...
Here, the y-axis shows the absolute run time to process one subband with
1.007~second of station samples from 64~stations in 8-bit mode.
...
The results are shown for the 8-bit sample mode, but the 16-bit and 4-bit mode
perform nearly the same, since only the performance of the the data receipt
from the I/O~node and data exchange phase are affected by the sample size, 
...
The FIR filter achieves 86\% and the hand-crafted 256-point FFT achieves 44\%
XXX

...
to study an extensive amount of new science cases~\footnote{See also \texttt{http://en.wikipedia.org/wiki/LOFAR}.}.
...
redshifted into the LOFAR sensitivity range.
...
the real-time part of the standard imaging pipeline.
...
The pipelines share common components, which shortens the development time for
new components, pipelines, and to implement the key science projects.
...
The samples from the received UDP packets are copied into a cicular buffer that
...
undreds of milliseconds (the WAN delay plus a safe margin) after $t_2$,
...
4~KB, generating a huge number of TLB-miss interrupts.
...
receieved from the I/O~nodes, how the data are exchanged between other compute
...
and time for each individual sample, costing another complex multiplication.
...
(\textbf{Chris: fix} ``Vienna'' FFTW achieves 34\%, which is also high for a FFT).

...
to be merely a bridge between the computation nodes and the outside world, but we configure them
with custom software in order to deal with the 3.1 Gb/s input, the output, and to perform both
pre- and post-processing on the data. Also, we use a custom network protocol called FCNP to be
obtain the required data rates to and from the compute nodes.

The computations themselves benefit from a mix of C++ and assembly. The C++ code base offers the flexibility

...
The software also supports multiple concurrent observations. %, even of different types.
...
saved us on costs. % TODO bedrag hier noemen?
...
% Veel te veel detail op deze plaats  --Rob
...
results were read by the I/O~node. % TODO Moet hier dan niet ook gewoon een circulaire buffer tussen? --Rob
...
4~subbands (the remaining two psets do not correlate).
Since there are 64~cores in one pset and an integration time equals 1.007~s,
the available time to process one chunk of data is 16.1~s.

Mode~\textsf{B} trades accuracy for subbands by reducing the sample size to
8~bits, doubling the number of subbands that can be sent by the stations.
For the correlator, this implies that the total input data rate remains the
same, but that the processing requirements and output data rate double.
Since each of the 62~psets that actually correlate now has to process
8~subbands, the available time is reduced to 8.03~s.

Mode~\textsf{C} uses 4-bit samples, and is only suitable for frequency
subbands that are free of RFI (otherwise, the bits are used to encode the RFI,
not the signal of interest).
This mode is planned for EoR observations, where the high number of subbands
is used to observe sky in about six directions simulaneously.
...
core stations.
The exact number of stations that the correlator will have to process is not
yet known, but is likely to be between 36 and~46.
We optimistically assume 48~stations for the performance measurements.


\subsection{Computational performance on the Compute Nodes}

\begin{figure}
\subfigure[As function of number of stations.  The input samples are 8~bit,
...
work load.
The y-axis shows the absolute run time to process one subband with
...
Figure~\ref{fig:cn-performance-func-stations} shows execution times for up
64~stations in a setting that is similar to observation mode~\textsf{B}.
The $O(n^2)$ complexity of the correlator is clearly visible, while other
components scale linearly with the number of stations.
Despite the high data rates, I/O~requires hardly any time.
Naturally, the time for input or output cannot exceed $1/64^\mathrm{th}$
...
The load on the compute nodes is 35\%, 70\%, and 84\%.
...
The FIR filter achieves 86\% FPU efficiency, and the hand-crafted 256-point FFT
44\%
...
I/O~node spend their time in various situations.
...
The I/O~nodes of the 62~psets that correlate receive and forward the samples
of one station (up to 3.1~Gb/s) and send the correlations of up to 8~subbands
to storage (up to 1.2~Gb/s).
The figure shows that most time spent in the receipt of UDP packets.
This load is partially independent of the number of subbands, since a lower
number of subbands decreases the packet size (down from 7,998~bytes), but not
the amount of packets (48,828~packets per second).
...
for the two other challenging modes as well.
...
In the 4-bit mode, the number of stations is limited to about~48 if only one
rack is used.
This is no problem, since this mode is only envisioned for particular
observations where only the core stations are used, of which there will
likely be 36 to~46.
Also, this mode was qualified for using two racks, but we are able to do so on
one rack only.
In this mode, the maximum number of subbands is~992.
Yet, the output data rate is only 13\%~higher than in the 64-station/8-bit mode,
due to the reduced number of stations.

...
\caption{A subset of the data flow for a beam formed by two stations, until the first exchange (left) and since the second exchange (right). The dotted lines depict chunks, and sb abbreviates subbands. The actual data flow involves 64 psets.}
\label{fig:dataflow}
\end{figure}
...
In the second all-to-all exchange, the chunks made by the beam former are again exchanged over the 3D-torus network. Due to memory constrains on the compute cores, the cores that performed the beam forming cannot be the same cores that receive the beam data after the exchange. We assign a set of cores (\emph{output cores}) to receive the chunks. The output cores are chosen before an observation, and are distinct from the \emph{input cores} which perform the earlier computations in the pipeline. Figure \ref{fig:dataflow} (right) shows a (partial) second exchange, in which two output cores receive chunks from all input cores.
...
\hline
\hline
\circlenumber{A} & Stokes I    & N & 12 &  4 & 543 &  12 Gb/s & 70 Gb/s & Memory & Surveys \\

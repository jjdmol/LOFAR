...
class Parset(util.Parset.Parset):
    ...
    def convertSASkeys(self):
	  ...
	  while "Observation.Beam[%s].angle1" % (nrBeams,) in self:
	    ...
	    beamSubbands = self.getInt32Vector("Observation.Beam[%s].subbandList" % (b,))
            beamBeamlets = self.getInt32Vector("Observation.Beam[%s].beamletList" % (b,))
            ...
            for n in xrange(len(beamSubbands)):
              beamlet = beamBeamlets[n] # the index of the subband (0..247)

              assert beamlet not in allSubbands, "Beam %d and %d both use beamlet %d" % (allSubbands[subbandNr]["beam"],b,beamlet)

              allSubbands[beamlet] = {
                "beam":     b,
                "subband":  beamSubbands[n],
                "beamlet":  beamlet,

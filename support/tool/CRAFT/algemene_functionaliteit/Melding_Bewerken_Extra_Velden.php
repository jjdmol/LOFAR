	...
	<body bgcolor="#B3CCE6">
		<?php
			...
			if (isset($_GET['c']) && $_GET['c'] != 0) {		
				//als o gelijk is aan n, dan is het type niet verandert dus gegevens uit de db laden
				if (isset($_GET['o']) && isset($_GET['n']) && $_GET['o'] == $_GET['n']) {
					$query = "SELECT Kolom_ID FROM melding_koppel_extra WHERE Meld_Lijst_ID = '". $_GET['c'] ."'";
					...
					$query = "SELECT Kolom_ID FROM type_melding_koppel_extra WHERE Meld_Type_ID in (SELECT Meld_Type_ID FROM melding_lijst WHERE Meld_Lijst_ID = '". $_GET['c'] ."')";
					$rest = mysql_query($query);
				...
				//o is niet gelijk aan n, dus is het type verandert dus alleen a velden maken
				else {
					echo("<form>\n");
					echo("<table>\n");
					$aantal_velden = 0;
					...
					$aan_te_maken = 0;
					//nog niet aangemaakte extra velden (die later toegevoegd zijn) toevoegen
					$query = "SELECT Kolom_ID FROM type_melding_koppel_extra WHERE Meld_Type_ID = '". $_GET['n'] ."'";
					$rest = mysql_query($query);
					while ($data = mysql_fetch_array($rest)) {
						$query = "SELECT * FROM extra_velden WHERE Kolom_ID = '". $data['Kolom_ID']  ."'";
						$result = mysql_query($query);

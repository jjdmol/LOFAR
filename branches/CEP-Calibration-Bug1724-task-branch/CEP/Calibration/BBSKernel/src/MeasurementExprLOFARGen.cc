...
void generateExpr(const BaselineSeq &baselines, const vector<Source::Ptr> &sources, const vector<DDEIndex> &index, const vector<ExprDDE::Ptr> &dde,
    const ExprDDE::Ptr &transform, unsigned int level, unsigned int start, unsigned int end, const vector<Expr<Vector<3> >::Ptr> &uvw, ExprDDEStack &stack, vector<Expr<JonesMatrix>::Ptr> &expr);
...
//            cout << "level: " << level << " generate for subrange [" << start << "," << pivot << "]" << endl;
            ...
            generateExpr(baselines, sources, index, dde, transform, level + 1, start, pivot, uvw, childStack, childExpr);
    ...
    const IonosphereExpr::Ptr &exprIonosphere)
{
    ...
    for(unsigned int i = 0; i < instrument->nStations(); ++i)
    {
        ...
        // patch.
        Expr<Vector<2> >::Ptr exprAzEl =
            makeAzElExpr(instrument->station(i)->position(), exprPosition);

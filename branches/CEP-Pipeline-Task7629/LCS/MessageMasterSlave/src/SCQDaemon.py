...
"""
Slave command queue. Extends the command queue interface with the 
...
"""
class SCQDaemon(CQDaemon.CQDaemon):
    def __init__(self, broker, busname, commandQueueName,
                 deadLetterQueueName, loop_interval=10, daemon=True):
        """
        
        """
        super(SCQDaemon, self).__init__(broker, busname, commandQueueName,
                 deadLetterQueueName, loop_interval, daemon)

        self._toBus = ToBus(self._busname, broker = self._broker)
    ...
    def close(self):
        ...
        super(CQDaemon, self).close() # then the superclass
    ...
    def _process_deadletter_queue(self):
        ...
        while True:
            ...
            if not unpacked_msg_data:  # if unpacking failed
                ...
                self._process_failed_run_job(unpacked_msg_content)
                self._fromDeadletterBus.ack(msg)                         

...
class ToBus(Session):
    ...
    def send(self, msg):
        try:
            logger.debug("[ToBus] Sending message to queue %s", self.queue)
            ...
            logger.debug("[ToBus] Message sent to queue %s", self.queue)
...
class FromBus(Session):
    ...
    def get(self, timeout=None):
        ...
        try:
            ...
            if receiver != None:
                logger.debug("[FromBus] Message available on queue %s", receiver.source)
                msg = receiver.fetch() # receiver.get() is better, but requires qpid 0.31+
                if msg is None:
                    logger.debug("[FromBus] Could not retrieve available message on queue %s", receiver.source)
                else:
                    logger.debug("[FromBus] Message received on queue %s", receiver.source)
    ...
    def ack(self, msg):
        ...
        logging.debug("[FromBus] Message ACK'ed");

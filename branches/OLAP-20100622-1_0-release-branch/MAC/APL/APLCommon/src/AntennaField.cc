...
AntennaField::AntennaField(const string& filename)
{
	...
	while(getline(inputStream, inputLine)) {
		if (inputLine.empty() || inputLine[0] == '\0' || inputLine[0] == ']' || inputLine[0] == '#') {
			continue;
		}	
		...
		if (fields.size() == 2) {
			...
			if (fields[0].find("NORMAL-VECTOR") != string::npos) {
				...
				ASSERTSTR(nVect.dimensions() == 1 && nVect.extent(firstDim) == 3, 
							"NormVector of field '" << fieldName << "' should be a 1 dimensional array with 3 values ,not " << nVect);
				LOG_DEBUG_STR(fieldName << " normvector is " << nVect);
				itsNormVectors[fieldIndex].resize(nVect.shape());
				...
				ASSERTSTR(rMat.dimensions() == 2 && rMat.extent(firstDim) == 3 && rMat.extent(secondDim) == 3, 
							"RotationVector of field '" << fieldName << "' should be a 3 dimensional array with 3 values ,not " << rMat);
				LOG_DEBUG_STR(fieldName << " rotationVector is " << rMat);
				itsRotationMatrix[fieldIndex].resize(rMat.shape());
				itsRotationMatrix[fieldIndex] = rMat;
				continue;
			}
			else {
...
int AntennaField::name2Index(const string& fieldName) const
{

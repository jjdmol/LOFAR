...
class NodeDeamon:
    ...
    def main_loop(self):
        self.input_sockets.append(self.socked_listener)
        continue_loop = True
        while continue_loop:
            time.sleep(delay)
            ss = select.select
            inputready, outputready, exceptready = ss(self.input_sockets, [], [])
    ...
    def on_accept(self):
        ...
        self.input_sockets.append(clientsock)

#        if forward:
#            print clientaddr, "has connected"
#            self.input_sockets.append(clientsock)
##            self.input_sockets.append(forward)
##            self.socked_frm_to_[clientsock] = forward
...
#            clientsock.close()
    ...
    def on_close(self):
        ...
        #remove objects from input_sockets
        self.input_sockets.remove(self.s)
        #self.input_sockets.remove(self.socked_from_to_[self.s])

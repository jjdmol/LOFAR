...
def calculate_and_print_stats(parsed_lines):
    """
...
def perform_analysis(filename, obs_id):
    ...
    calculate_and_print_stats(parsed_lines)
...
##Storage_main@locus008 03-07-12 09:55:15.988 INFO  [obs 60719 type 1 stream  45 writer   3] [OutputThread] Finished writing: 514 blocks written, 81 blocks dropped: 13.6% lost
import subprocess
import re
import numpy
import sys
...
def print_stats(stats):
    ...
    """
    formatted_line = \
      "{0}-> min:{1}  max:{2}  mean:{3}  std:{4}  median:{5}".format(
         stats['name'], stats['min'], stats['max'], stats['mean'], stats['std'],
           stats['median'])

...
-- GENERIC FUNCTION --
CREATE OR REPLACE FUNCTION createNewRecord(VARCHAR, INT4, VARCHAR)
RETURNS INT4 AS $$
	DECLARE
		vNewID		INT4;
	...
	BEGIN
	  ...
	  vQuery:='INSERT INTO ' || aBasename || 'Table (recordID,treeID,nodename) values (' || vNewID || ',' || $2 || E',\'' || $3 || E'\')';
	  EXECUTE vQuery;
	  RETURN vNewID;
	END;
$$ language plpgsql;
...
CREATE OR REPLACE FUNCTION textValue(BOOLEAN)
RETURNS TEXT AS $$
    BEGIN
      IF $1 THEN
        RETURN 'true';

//# LofarATermBeam.cc: Compute the LOFAR beam response on the sky.
//#
//# Copyright (C) 2011
...
namespace LOFAR
{
  LofarATermBeam::LofarATermBeam(const MeasurementSet& ms,
  ...
  vector<Cube<Complex> > LofarATermBeam::evaluate(const IPosition &shape,
    ...
    bool normalize)
  {
    ...
    // Compute ITRF map.
    LOG_INFO("LofarATermBeam::evaluate(): Computing ITRF map...");
    Cube<double> mapITRF = computeITRFMap(coordinates, shape, convertor);
    LOG_INFO("LofarATermBeam::evaluate(): Computing ITRF map... done.");

    // Compute element beam response.
    LOG_INFO("LofarATermBeam::evaluate(): Computing station response...");
...
//    AlwaysAssert(false, SynthesisError);
    ...
    LOG_INFO("LofarATermBeam::evaluate(): Computing station response... done.");
  ...
  double LofarATermBeam::resolution()
  {
    Double station_diam = 70.;                                           // station diameter in meters: To be adapted to the individual station size.
    return ((casa::C::c/m_refFreq)/station_diam)/2.;
  }

  Array<DComplex> LofarATermBeam::normalize(const Array<DComplex> &response)
    const
  {
  ...
  Array<DComplex> LofarATermBeam::evaluateElementBeam(const BeamCoeff &coeff,
  ...
  Array<DComplex> LofarATermBeam::evaluateStationBeam(const Station &station,
    ...
    const Vector<Double> &freq) const
  {
    ...
    for(uint i = 0; i < station.nField(); ++i)
    {
      ...
      LOG_INFO("LofarATermBeam::computeStationBeam: Computing element beam...");
      Array<DComplex> beam;
      if(field.isHBA())
      {
      ...
      LOG_INFO("LofarATermBeam::computeStationBeam: Computing element beam... done.");

      if(field.isHBA())
      {
        // Compute tile array factor.
        LOG_INFO("LofarATermBeam::computeStationBeam: Computing tile array factor...");
        Cube<DComplex> tileAF = evaluateTileArrayFactor(field, refTile, map,
          freq);
        LOG_INFO("LofarATermBeam::computeStationBeam: Computing tile array factor... done.");
      ...
      LOG_INFO("LofarATermBeam::computeStationBeam: Computing station array factor...");
      ...
      LOG_INFO("LofarATermBeam::computeStationBeam: Computing station array factor... done.");
  ...
  Cube<DComplex> LofarATermBeam::evaluateTileArrayFactor(const AntennaField &field,
  ...
  Cube<double> LofarATermBeam::computeITRFMap(const DirectionCoordinate &coordinates,
  ...
  void LofarATermBeam::initInstrument(const MeasurementSet &ms)
  {
  ...
  Station LofarATermBeam::initStation(const MeasurementSet &ms,
    uint id,
    const String &name,
  ...
  void LofarATermBeam::initReferenceDirections(const MeasurementSet &ms,
    uint idField)
  {
  ...
  void LofarATermBeam::initReferenceFreq(const MeasurementSet &ms,

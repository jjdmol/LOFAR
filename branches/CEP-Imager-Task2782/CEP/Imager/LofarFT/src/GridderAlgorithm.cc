//# GridderAlgorithm.cc:class providing for parallelized residualimagemaking
...
GridderAlgorithm::GridderAlgorithm() : myName_p("GridderAlgorithm")
{
...
//   weight_p->resize();
  ...
  int result = 0;
  applicator.put(result);
  

};

Bool GridderAlgorithm::post(int &args) {

  int rank;
  Bool assigned = applicator.nextAvailProcess( *this, rank );
  if (assigned) {
    // applicator.put(args)
  }
  return assigned;
...
Bool GridderAlgorithm::fetch(int &result) {
  Bool allDone;
  applicator.nextProcessDone( *this, allDone);
  if (!allDone) {
    applicator.get(result);
  }
  return allDone;
};


String& GridderAlgorithm::name(){
...
void GridderAlgorithm::initializePut(const VisBuffer& vb, Int model) {
//   ft_p->initializeToSky(*cImage_p, *weight_p,vb);
  // Need to add support for varying SkyJones
}


void GridderAlgorithm::putResidualVis(VisBuffer& vb, Int model) {
//     Bool dopsf=False;
    // Need to add support for varying SkyJones
//     ft_p->put(vb, -1, dopsf);
}

...
#undef LOG_INFO_STR
#define LOG_INFO_STR(a) cout << a << endl;
...
struct FailedInfo
{
  FailedInfo (const string& station, const string& time,
...
typedef vector<FailedInfo> VectorFailed;

// Read the broken hardware file giving a map of antenna name to a vector
// of pairs of RCUnr and time.
map<string, VectorFailed> readFile (const string& fileName,
                                    double startTime, double endTime)
{
  map<string, VectorFailed> brokenHardware;
        ...
        if (parts.size() > 1  &&  parts[1].substr(0,3) == "RCU") {
          ...
          if (epoch.get() >= startTime  &&  epoch.get() < endTime) {
            ...
            brokenHardware[parts[0]].push_back
              (FailedInfo(parts[0], date+'/'+time.substr(0,8), epoch, rcu));
          }
        }
      }
    }
...
vector<VectorFailed> antennaConvert
(const Table& ms, const map<string,VectorFailed>& broken)
{
                  ...
                  const vector<VectorFailed>& brokenDuring)
{
  ...
  for (uInt row=0; row<antFieldTab.nrow(); ++row) {
    ...
    if (!brokenBefore[antId].empty()  ||  !brokenDuring[antId].empty()) {
      ...
      for (VectorFailed::const_iterator iter = brokenBefore[antId].begin();
      ...
      for (VectorFailed::const_iterator iter = brokenDuring[antId].begin();
...
void failedTiles2MS (const string& msName, const string& beforeName,
...
int main (int argc, char* argv[])
{
  try {
    ...
    failedTiles2MS (msName, beforeName, duringName);

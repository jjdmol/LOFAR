...
VHECRsettings::VHECRsettings(ParameterSet* aParSet)
{
    ...
    if (globalAntennaSets()->usesLBAfield(antennaSet)) {
        antennaField = "LBA";
    }
    else {
        antennaField = "HBA";
    ...
    // [rcu, xyz]
    rcuPosITRF.resize(globalAntennaField()->RCUPos(antennaField).shape());
    rcuPosITRF = globalAntennaField()->RCUPos(antennaField);
...
ostream& VHECRsettings::print(ostream&  os) const
{
    ...
    os << "rcuPosITRF       : " << rcuPosITRF << endl;
    
    os << "clock freq       : " << clockFreq << " MHz" << endl;

...
size_t lengthActivationsData = NR_BASENETWORKS* NR_TIMESTEPS * NR_SOLUTION_PARALEL * NR_NEURONS * COMPLEX;
                   ...
                   string function)
{
  ...
  unsigned lengthRandomData = 128;
  size_t sizeRandomData = lengthRandomData* sizeof(curandState);
  DeviceMemory devRandomMemory(ctx, sizeRandomData);
  cuStream.writeBuffer(devActivationsMemory, rawActivationsData);
  ...
  hKernel.setArg(4, devRandomMemory);
  ...
  Block localWorkSize(NR_SOLUTION_PARALEL, 1, 1);
...
int main()
{
  ...
  for (unsigned idx_basenetworks = 0; idx_basenetworks < NR_BASENETWORKS; ++ idx_basenetworks)
  {
    unsigned start_basenetwork_idx = idx_basenetworks * NR_TIMESTEPS * NR_SOLUTION_PARALEL * NR_NEURONS * COMPLEX;
    for (unsigned idx_timestep = 0; idx_timestep < NR_TIMESTEPS; ++ idx_timestep)
    {
      unsigned start_timestep_idx = idx_timestep * NR_SOLUTION_PARALEL * NR_NEURONS * COMPLEX;
      cout << "timestep " << idx_timestep << " |";
      for (unsigned idx_solutions =0; idx_solutions < NR_SOLUTION_PARALEL;++idx_solutions)
      {
        unsigned start_solutions_idx = idx_solutions * NR_NEURONS * COMPLEX;

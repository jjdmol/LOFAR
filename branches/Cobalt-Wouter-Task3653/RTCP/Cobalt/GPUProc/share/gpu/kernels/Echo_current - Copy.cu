                     ...
                     const void *baseWeightsTypePtr) 
{
  ...
  unsigned tid = threadIdx.x * threadDim.x + threadIdx.y;
  ...
  curandState_t *state;
  curandinit(blockIdx.x, tid,0, state);  //maybee this can be done bettew: salt in the randomization
  __shared__ complexf weightRandom[NR_NEURONS + 1][NR_NEURONS]; // use 512 complex and shift through them. this alles omition of 
  ...
  for (unsigned idx_copy = 0; idx_copy < NR_NEURONS * NR_NEURONS / NR_SOLUTION_PARALEL; ++ idx_copy)
  {
    ...
    if ( idx > NR_NEURONS * NR_NEURONS )
      break;
    weightRandom[idx /  NR_NEURONS][idx % NR_NEURONS] = 
          complexf(curand_normal(state), curand_normal(state));  //lognormal aslo candidate?
  }
  __syncthreads();
    ...
    for (unsigned idx_neuron = 0; idx_neuron < NR_NEURONS; ++ idx_neuron)
    {
      ...
      for (unsigned idx_synaps = 0; idx_synaps < NR_NEURONS; ++ idx_synaps)
      {
        neuron_activation += shared_baseWeights[idx_neuron][idx_synaps] * 
           weightRandom[saved_neuron_idx][(idx_neuron + threadIdx.y) % NR_NEURONS] *

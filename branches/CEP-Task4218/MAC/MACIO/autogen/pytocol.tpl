...
[+ DEFINE event_param_format2 +][+ CASE (get "type") +][+ ==* char +]c[+ ==* uint16 +]H[+ ==* int16 +]h[+ ==* uint32 +]I[+ ==* int32 +]i[+ ==* uint64 +]L[+ ==* int64 +]l[+ ==* float +]f[+ ==* double +]d[+ == bool +]b[+ ESAC +][+ ENDDEF +]
...
#
[+ FOR error "" +][+ prefix_ucase +]_[+ (get "id") +]_ERR = [+ (for-index) +]
[+ ENDFOR error +]

#
# Define protocol message types
#
[+ FOR event "" +][+ signal_id +] = [+ (for-index) +]
[+ ENDFOR event +]
...
class [+ event_class_decl +]
  ...
  def unpack(self):
    offset = GCFEvent.unpack(self)[+ FOR param +]
...
[+ FOR event "" +]  obj[+ (for-index) +] = [+ event_class_name +]()
  print obj[+ (for-index) +]
  obj[+ (for-index) +].pack()
  print obj[+ (for-index) +]
  obj[+ (for-index) +].unpack()
  print obj[+ (for-index) +]
[+ ENDFOR event +]

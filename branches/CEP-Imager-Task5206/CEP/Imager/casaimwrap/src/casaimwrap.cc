...
{
    MeasurementSet                          ms;
    CountedPtr<LOFAR::LofarFT::FTMachine>              ft;

    CountedPtr<LOFAR::LofarFT::ConvolutionFunction>    cf;
    Record                                  options;
    ...
    const ValueHolder &image)
{
    ...
    StokesImageUtil::changeCStokesRep(linearImage, StokesImageUtil::LINEAR);
...
// 
//     typedef CountedPtr<LOFAR::LofarFT::ConvolutionFunction> PtrType;
//     context.cf = PtrType(new LOFAR::LofarFT::ConvolutionFunction(im_shape,
...
// 
//     LOFAR::LofarFT::CFStore store = context.cf->makeConvolutionFunction(threadId, stationA,

...
void LofarCubeSkyEquation::makeApproxPSF(PtrBlock<ImageInterface<Float> * >& psfs) 
{
...
void LofarCubeSkyEquation::makeMosaicPSF(PtrBlock<ImageInterface<Float> * >& psfs){
...
void LofarCubeSkyEquation::makeSimplePSF(PtrBlock<ImageInterface<Float> * >& psfs) {
...
void
LofarCubeSkyEquation::configureAsyncIo (ROVisibilityIterator * & oldRvi, VisibilityIterator * & oldWvi)
{
...
void LofarCubeSkyEquation::initializePutSlice(const VisBuffer& vb, Bool dopsf,
					 Int cubeSlice, Int nCubeSlice) {
  AlwaysAssert(ok(),AipsError);

  LogIO os(LogOrigin("LofarCubeSkyEquation", "initializePutSlice"));
...
void LofarCubeSkyEquation::oldInitializePutSlice(const VisBuffer& vb, Bool /*dopsf*/, 
					 Int cubeSlice, Int nCubeSlice) {
  AlwaysAssert(ok(),AipsError);
...
void LofarCubeSkyEquation::newInitializePutSlice(const VisBuffer& vb, Bool dopsf, 
					 Int cubeSlice, Int nCubeSlice) {
  AlwaysAssert(ok(),AipsError);
  Bool dirDep= (ej_ != NULL);
...
void LofarCubeSkyEquation::getWeightImage(Int model, ImageInterface<Float>& im){
...
Bool LofarCubeSkyEquation::isNewFTM(FTMachine* ftm)
{
  return (
...
Bool LofarCubeSkyEquation::isNewFTM()
{
  AlwaysAssert( &(*ftm_p[0]) ,AipsError );
  return isNewFTM(&(*ftm_p[0]));
}

void LofarCubeSkyEquation::finalizePutSlice(const VisBuffer& vb,  Bool dopsf,
				       Int cubeSlice, Int nCubeSlice) 
{
  //============================================================================
  // NEW CODE BEGINS
  // Iterate across fields
  LogIO os(LogOrigin("LofarCubeSkyEquation", "finalizePutSlice"));
					  ...
					  Int cubeSlice, Int nCubeSlice) 
{
  ...
  // NEW CODE BEGINS
  // Iterate across fields
  LogIO os(LogOrigin("LofarCubeSkyEquation", "newFinalizePutSlice"));
    {
...
void LofarCubeSkyEquation::oldFinalizePutSlice(const VisBuffer& vb,  Bool /*dopsf*/,
					  Int cubeSlice, Int nCubeSlice) 
{
...
void LofarCubeSkyEquation::initializeGetSlice(const VisBuffer& vb, 
					   ...
					   Int nCubeSlice)
{
  LogIO os(LogOrigin("LofarCubeSkyEquation", "initializeGetSlice"));
...
void LofarCubeSkyEquation::newInitializeGetSlice(const VisBuffer& vb, 
					    Int /*row*/, 
					    Bool incremental, Int cubeSlice, 
					    Int nCubeSlice)
{
  ...
  for(Int field=0; field < nfields; ++field)
      ...
      for(Int taylor=0; taylor < sm_->numberOfTaylorTerms(); ++taylor)
	{
      ...
      //U// cout << "LofarCubeSkyEquation :: Calling new initializeToVis with " << modelSliceVec.nelements() << " models and " << imGetSliceVec.nelements() << " complex grids " << endl;
...
void LofarCubeSkyEquation::oldInitializeGetSlice(const VisBuffer& vb, 
					    Int row, 
					    Bool incremental, Int cubeSlice, 
					    Int nCubeSlice){
  sm_->setImageNormalization(False);
...
void LofarCubeSkyEquation::tmpWBNormalizeImage(Bool& dopsf, const Float& pbLimit)
{
  LogIO os(LogOrigin("LofarCubeSkyEquation", "tmpNormalizeImage"));

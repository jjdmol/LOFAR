  ...
  const casa::VisBuffer& vb) const
{
    ...
    casa::Vector<casa::Double> fvec;
    casa::Matrix<casa::Double> uvwmat;

    if (doFilter() || type == "uniform" || type == "radial"){
        fvec = vb.frequency ();
        uvwmat = vb.uvwMat ();
    }
    ...
    if (getType () == "uniform") 
    {
        weightUniform (imagingWeight, flagmat, uvwmat, fvec, weightvec, vb.msId (), vb.fieldId ());
    } 
    else if (getType () == "radial") 
    {
        weightRadial (imagingWeight, flagmat, uvwmat, fvec, weightvec);
    }
    else 
    {
    ...
    if (doFilter ()) 
    {
        filter (imagingWeight, flagmat, uvwmat, fvec, weightvec);
    }

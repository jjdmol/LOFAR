  ...
  itsVerbose(parameters.asInt("verbose"))
{
  ...
  if (itsApplyIonosphere) 
  {
    ...
    cout << itsCal_pp_names << endl;
  }
}

void ATerm::initParmDB(const casa::String  &parmdbname)
{
...
void ATerm::setDirection(const casa::DirectionCoordinate &coordinates, const IPosition &shape) {
...
double ATerm::get_parmvalue( Record& parms, std::string parmname ) 
{
  double r = 0.0;
...
ATerm::ITRFDirectionMap
ATerm::makeDirectionMap(const DirectionCoordinate &coordinates,
...
vector<Cube<Complex> > ATerm::evaluate(uint idStation,
...
Cube<DComplex> ATerm::evaluateStationScalarFactor(const uint idStation,
...
vector<Matrix<Complex> > ATerm::evaluateArrayFactor(uint idStation,
  uint idPolarization,
...
vector<Cube<Complex> > ATerm::evaluateElementResponse(uint idStation,
...
Cube<DComplex> ATerm::evaluateIonosphere(uint station, const Vector<Double> &freq) const
{
  const ITRFDirectionMap &map = itsITRFDirectionMap;
...
//   template<>
//   Array<DComplex>
//   computeElementResponse<ElementHBA>(const LOFAR::BBS::AntennaField::ConstPtr &field,
//     const LOFAR::LofarFT::ATerm::ITRFDirectionMap &map, const Vector<Double> &freq);
// 
//   template<>
//   Array<DComplex>
//   computeElementResponse<ElementLBA>(const LOFAR::BBS::AntennaField::ConstPtr &field,
//     const LOFAR::LofarFT::ATerm::ITRFDirectionMap &map, const Vector<Double> &freq);
// 

...
namespace LofarFT {
  ...
  template
  void VisResampler::lofarDataToGrid_interp(
    Array<Complex>& grid,  VBStore& vbs,
    const Vector<uInt>& rows,
    Matrix<Double>& sumwt,
    const Bool& dopsf,
    CFStore& cfs);
						 ...
						 //vector<Float> wvec, Float wStep, Float wcf, vector<Complex> vecCorr)
  {
    ...
    Int sampx = SynthesisUtils::nint (cfs.itsSampling[0]);
    Int sampy = SynthesisUtils::nint (cfs.itsSampling[1]);
    Int supx = cfs.itsXSupport[0];
    Int supy = cfs.itsYSupport[0];
  ...
  void VisResampler::lofarGridToData_interp(
    VBStore& vbs,
  ...
  void VisResampler::lofarGridToData_linear(
    VBStore& vbs,
    const Array<Complex>& grid,
    const Vector<uInt>& rows,
    Int rbeg, Int rend,
...
///                                         << igrdpos[0] + igrdpos[1]*gridInc_p[1] + igrdpos[2]*gridInc_p[2] +igrdpos[3]*gridInc_p[3]<<endl;
  ...
  */
  ...
  //
  void VisResampler::lofarGridToData(
    VBStore& vbs,
    const Array<Complex>& grid,
    const Vector<uInt>& rows,
    Int rbeg, 
  ...
  */
  ...
  void VisResampler::lofarComputeResiduals(VBStore& vbs)
  {
  ...
  void VisResampler::sgrid(Vector<Double>& pos, Vector<Int>& loc,

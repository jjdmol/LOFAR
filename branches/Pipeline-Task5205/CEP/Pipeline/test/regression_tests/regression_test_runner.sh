...
WORKING_DIR=$"/data/scratch/$USER/regression_test_runner_$PIPELINE"     # Default working space
...
cp installed/share/pipeline/pipeline.cfg  $"$WORKING_DIR/pipeline.cfg"
...
cp /data/lofar/testdata/CEP/Pipeline/calibrator_pipeline/Observation64405 $"$WORKING_DIR/calibrator.parset"

python installed/bin/msss_calibrator_pipeline.py $"$WORKING_DIR/calibrator.parset" -c $"$WORKING_DIR/pipeline.cfg" -d

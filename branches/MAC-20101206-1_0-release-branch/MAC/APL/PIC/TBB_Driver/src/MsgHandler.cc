...
//-----------------------------------------------------------------------------
void MsgHandler::sendSavedTrigger(int boardnr)
{
	//LOG_DEBUG_STR(formatString("send saved trigger from board %d to client", boardnr));
	TBBTriggerEvent tbb_event;
	
	int channel = TS->getTriggerInfo()->boardchannel + (boardnr * TS->nrChannelsOnBoard());	
	TS->convertCh2Rcu(channel, &tbb_event.rcu);
	...
	tbb_event.missed          = TS->getTriggerInfo()->missed;

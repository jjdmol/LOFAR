...
//    os << "stations     : "; writeVector(os, stations, ",", "[", "]"); os << endl;
	...
	for (size_t	i(0) ; i < streamsToStorage.size(); i++) {
		StreamToStorage	sp = streamsToStorage[i];
		os << formatString("dp=%s, streamNr=%d, filename=%s, sourcePset=%d, destStorageNode=%s, destDir=%s\n",
				sp.dataProduct.c_str(), sp.streamNr, sp.filename.c_str(), sp.sourcePset, sp.destStorageNode.c_str(), sp.destDirectory.c_str());
	}

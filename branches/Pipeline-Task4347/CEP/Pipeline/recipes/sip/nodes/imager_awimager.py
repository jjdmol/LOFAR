...
class imager_awimager(LOFARnodeTCP):
    ...
    def _calc_par_from_measurement(self, measurement_set, parset, autogenerate_parameters):
        """
        ...
        # If we are not auto calculating get the fov from the parset
        if not autogenerate_parameters:
            fov = parset_object.getFloat('fov')
            if fov == 0.0:
                raise PipelineException("Tried reading field of view from parset"
                   " entrie was empty or 0.0: invalid value.")

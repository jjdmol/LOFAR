//# JonesCMul3.cc: Calculate left*mid*conj(right)
//#
//# Copyright (C) 2005
...
JonesCMul3::JonesCMul3 (const JonesExpr& left,
                  const JonesExpr& mid,
...
v22_i[_i]=_mm_add_pd(_mm_sub_pd(_mm_mul_pd(t21_i,r21_r),_mm_mul_pd(t21_r,r21_i)),_mm_sub_pd(_mm_mul_pd(t22_i,r22_r),_mm_mul_pd(t22_r,r22_i)));
  ...
  const Result *pvSet[N_PValues] =
    {&l11, &l12, &l21, &l22, &m11, &m12, &m21, &m22, &r11, &r12, &r21, &r22};
  ...
  {  
    ...
    } else if(pvIter.hasPValue(PV_LEFT11) || pvIter.hasPValue(PV_LEFT12)) {
      eval11 = eval12 = true;
    } else if(pvIter.hasPValue(PV_LEFT21) || pvIter.hasPValue(PV_LEFT22)) {
      eval21 = eval22 = true;
    } else if(pvIter.hasPValue(PV_RIGHT11) || pvIter.hasPValue(PV_RIGHT12)) {
      eval11 = eval21 = true;
    } else if(pvIter.hasPValue(PV_RIGHT21) || pvIter.hasPValue(PV_RIGHT22)) {
      eval12 = eval22 = true;
    }
...
{
// static NSTimer timer("JonesCMul3::getResult", true);
      ...
      if (eval11 || eval12) {
        ...
        if (eval11) {
          result11.setPerturbedValue(pvIter.key(),
            t11 * conj(mr11) + t12 * conj(mr12));
        }
        if (eval12) {
          result12.setPerturbedValue(pvIter.key(),
            t11 * conj(mr21) + t12 * conj(mr22));
        }
      }
      ...
      if (eval21 || eval22) {
        ...
        if (eval21) {
          result21.setPerturbedValue(pvIter.key(),
            t21 * conj(mr11) + t22 * conj(mr12));
        }
        if (eval22) {
          result22.setPerturbedValue(pvIter.key(),
            t21 * conj(mr21) + t22 * conj(mr22));
        }
      }

...
void ParmDBCasa::createTables (const string& tableName)
{
  ...
  ///tdnam.addColumn (ScalarColumnDesc<String>("NAMEX"));
  ///tdnam.addColumn (ScalarColumnDesc<String>("NAMEY"));
...
//     // Sort the Boxes indirectly (thus result is in index vector).
//     Vector<uInt> index;
//     GenSortIndirect<Box>::sort (index, domains.begin(), domains.size());
//     // Copy to std::vector.
//     indexVector.resize (index.size());
//     objcopy (indexvector.begin(), index.data(), index.size());
    // Construct the grid.
...
// ParmNameInfo ParmDBCasa::getInfo (const std::string& parmName)
// {
//   Table table = itsTables[1];
//   TableLocker locker(table, FileLocker::Read);
//   table = table(table.col("NAME") == parmName);
//   ParmNameInfo info;
//   Array<Bool> mask;
//   if (table.nrow() == 0) {
//     ParmValueSet pset = getDefValue (parmName, ParmValue());
//     info.itsParmId = -1;
//     info.itsType = pset.getType();
//     info.itsNrX  = pset.getParmValue(0).nx();
//     info.itsNrY  = pset.getParmValue(0).ny();
//     mask = pset.getSolvableMask();
//   } else {
//     ASSERT (table.nrow() == 1);
//     info.itsParmId = ROScalarColumn<Int>(table,"ID")(0);
//     info.itsType   = ParmNameInfo::FunkletType(ROScalarColumn<Int>(table,"TYPE")(0));
//     info.itsNrX    = ROScalarColumn<Int>(table,"NX")(0);
//     info.itsNrY    = ROScalarColumn<Int>(table,"NY")(0);
//     info.itsPerturbation = ROScalarColumn<Double>(table,"PERTURBATION")(0);
//     info.itsPertRel      = ROScalarColumn<Bool>(table,"PERT_REL")(0);
//     mask = ROArrayColumn<Bool>(table,"SOLVABLE");
//   }
//   if (mask.nelements() == 0) {
//     info.itsSolvableCoeff = info.itsNrX * info.itsNrY;
//   } else {
//     info.itsSolvableMask  = new Matrix<Bool>(mask);
//     info.itsSolvableCoeff = ntrue(mask);
//   }
//   return info;
// }

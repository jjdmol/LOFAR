...
extern long time_adjust;	// REO: is now in time.c
extern int tickadj;		// REO: is now in time.c
#include <linux/l_fp.h>
extern l_fp time_adj;		// REO: is now in time.c and is 64 bit
long time_next_adjust;		// REO: do need this one
...
// REO iso 90 lines below from PATCH
#endif
...
//		printk(KERN_INFO
//		       "hardpps: delta_nsec = %ld, time_adj = %lld", delta_nsec, time_adj);
#endif
#ifndef REO_MOEILIJK
//	ret += (s64)time_adj << (TICK_LENGTH_SHIFT - (SHIFT_SCALE - 10));
	ret += (s64)time_adj << (TICK_LENGTH_SHIFT - (SHIFT_NSEC));
#endif
	return ret;
}
// PATCH
...
//      printk(KERN_INFO "update_wall_time()");
#endif

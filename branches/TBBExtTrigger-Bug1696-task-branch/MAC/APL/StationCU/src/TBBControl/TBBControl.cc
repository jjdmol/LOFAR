...
GCFEvent::TResult TBBControl::active_state(GCFEvent& event, GCFPortInterface& port)
{
	...
	// TODO TBB
		...
		case CR_STOP:{
			...
			StopRequest stopRequest;
			LOG_INFO_STR("Ext.Trig. Stop recording");
			...
			for (iter = stopevent.stopVector.requests.begin(); iter != stopevent.stopVector.requests.end(); iter++) {
				...
				LOG_INFO_STR("rcuSet=" << stopRequest.rcuSet);
				LOG_INFO_STR("stopTime=" << stopRequest.stopTime);
			}
			CRStopAckEvent ack;
		...
		case CR_READ:{
			...
			ReadRequest readRequest;
			RCUset_t rcuSet;
			LOG_INFO_STR("Ext.Trig. Start dumping CEP data");
		...
		case CR_RECORD:{
			...
			RCUset_t rcuSet;
			LOG_INFO_STR("Ext.Trig. Start recording");
		...
		case CR_CEP_SPEED:{
			...
			LOG_INFO_STR(formatString("Ext.Trig. CEP speed changed: delay= %d nsec, number of datapaths= %d", itsCepDelay*5, itsMaxCepDatapaths));
		...
		case CR_STOP_DUMPS:{
			...
			// stop dumping cep data
			itsReadRequests.clear();
			LOG_INFO_STR("Ext.Trig. Stop dumping CEP data");
...
RCUset_t TBBControl::strToBitset(string inputstring)
{
	...
	LOG_INFO_STR(formatString("selection=%s",start));
	resultset.reset();

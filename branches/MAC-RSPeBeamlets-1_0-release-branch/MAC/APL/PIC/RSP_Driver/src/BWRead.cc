...
void BWRead::sendrequest()
{
  ...
  switch (m_regid)
  {
    case MEPHeader::BF_XROUT:
      bfcoefs.hdr.set(MEPHeader::BF_XROUT_HDR, 1 << m_blp, MEPHeader::READ, size, m_offset);
      break;
    case MEPHeader::BF_XIOUT:
      bfcoefs.hdr.set(MEPHeader::BF_XIOUT_HDR, 1 << m_blp, MEPHeader::READ, size, m_offset);
      break;
    case MEPHeader::BF_YROUT:
      bfcoefs.hdr.set(MEPHeader::BF_YROUT_HDR, 1 << m_blp, MEPHeader::READ, size, m_offset);
      break;
    case MEPHeader::BF_YIOUT:
      bfcoefs.hdr.set(MEPHeader::BF_YIOUT_HDR, 1 << m_blp, MEPHeader::READ, size, m_offset);
      break;
  }
...
GCFEvent::TResult BWRead::handleack(GCFEvent& event, GCFPortInterface& /*port*/)
{
	...
	Array<complex<int16>, 2> weights((complex<int16>*)&bfcoefs.coef, shape(elem_size, MEPHeader::N_POL), neverDeleteData);

	uint8	beamDirection = 0;
	...
	if (0 == GET_CONFIG("RSPDriver.LOOPBACK_MODE", i)) {
		...
		//
		weights -= Cache::getInstance().getBack().getBeamletWeights()()(0, Range(global_blp * 2, global_blp * 2 + 1), 
																		beamDirection, target_range);
		...
		// X
		Cache::getInstance().getBack().getBeamletWeights()()(0, global_blp * 2, beamDirection, target_range) = 
					weights(Range::all(), 0);

		// Y
		Cache::getInstance().getBack().getBeamletWeights()()(0, global_blp * 2 + 1, beamDirection, target_range) = 
					weights(Range::all(), 1);
	}

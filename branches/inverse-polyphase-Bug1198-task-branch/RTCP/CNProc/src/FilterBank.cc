...
FilterBank::FilterBank(const bool verbose, const unsigned taps,
		const unsigned channels, const WindowType windowType) :
	itsWindowType(windowType), itsNrTaps(taps), itsNrChannels(channels),
			itsVerbose(verbose) {
	generate_filter();
}

FilterBank::FilterBank(const bool verbose, const unsigned taps,
		const unsigned channels, float* newWeights) :
	itsWindowType(PREDEFINED_FILTER), itsNrTaps(taps), itsNrChannels(channels),
			itsVerbose(verbose) {
	weights.resize(boost::extents[itsNrChannels][itsNrTaps]);
	memcpy(weights.origin(), newWeights, (itsNrChannels * itsNrTaps)
			* sizeof(float));
}
...
double FilterBank::besselI0(const double x) {
	...
	const double p1 = 1.0, p2 = 3.5156229, p3 = 3.0899424, p4 = 1.2067492, p5 =
			0.2659732, p6 = 3.60768e-2, p7 = 4.5813e-3;

	const double q1 = 0.39894228, q2 = 1.328592e-2, q3 = 2.25319e-3, q4 =
			-1.57565e-3, q5 = 9.16281e-3, q6 = -2.057706e-2, q7 = 2.635537e-2,
			q8 = -1.647633e-2, q9 = 3.92377e-3;
	...
	if (ax < k1) {
		double xx = x / k1;
		y = xx * xx;
		result = p1 + y * (p2 + y * (p3 + y * (p4 + y
				* (p5 + y * (p6 + y * p7)))));
	} else {
		y = k1 / ax;
		result = (exp(ax) / sqrt(ax)) * (q1 + y * (q2 + y * (q3 + y * (q4 + y
...
void FilterBank::interpolate(const double* x, const double* y,
		const unsigned xlen, const unsigned n, double* result) {
	unsigned nextX = 0;
	unsigned index = 0;

	for (double interpolatedX = 0; interpolatedX <= 1.0; interpolatedX += 1.0
			/ (n - 1), index++) {
...
void FilterBank::generate_fir_filter(const unsigned n, const double w,
	...
	double f[] = { 0.0, w - ramp_n / grid_n / 2.0, w,
			w + ramp_n / grid_n / 2.0, 1.0 };
...
#if defined HAVE_FFTW3
	fftwf_complex* cinput = (fftwf_complex*) fftwf_malloc(grid_n * 4
			* sizeof(fftwf_complex));
	fftwf_complex* coutput = (fftwf_complex*) fftwf_malloc(grid_n * 4
			* sizeof(fftwf_complex));
#elif defined HAVE_FFTW2
...
void FilterBank::generate_filter() {
	...
	if (itsVerbose) {
		logStr << "generating FIR filter bank with " << itsNrChannels
				<< " channels and " << itsNrTaps << " taps (" << n
				<< " total), using a ";
	}

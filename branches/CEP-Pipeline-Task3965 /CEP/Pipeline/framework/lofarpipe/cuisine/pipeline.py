...
class pipeline_manager(WSRTrecipe):
    ...
    def communicate_job(self, job):
        ...
        try:
            if not isinstance(self.client, types.NoneType):
                ...
                if status: ## we retry, because the client does not do an internal retry, but only reports the problem
                    ...
                    while (status and (count < 10)):
                        self.logger.info("Got some error, retrying " + str(job['ExportID']) + ": " + message)
                        time.sleep(60)
                        (status, message) = self.client.setStatus(str(job['ExportID']), str(job['Status']))
                        count += 1
                if status:
            ...
            self.logger.error('Could not update job %s status to %s.' % (str(job['ExportID']), str(job['Status'])))
    ...
    def new_job(self, filename, fileContent):
        ...
        try:
            ...
            self.logger.debug('file not found (existing job?): ' +
                             self.inputs['NewJobsDirectory'] + '/' + filename)
        ...
        if job['Status'] == JobScheduled:
            ...
            self.logger.info('Parsing ' + self.inputs['JobsDirectory'] + '/' + filename + ' failed') ## Not implemented yet
    ...
    def update_job(self, exportID, status):
        ...
        if j:
            ...
            self.logger.debug('Job ' + str(exportID) +
    ...
    def next_job(self):
        ...
        for j in self.jobs:
            if j['Status'] >= JobScheduled:
                if j['Status'] > JobScheduled:
                    self.logger.info('Restarting job: ' + j['ExportID'])
    ...
    def cook_job(self, job):
        ...
        if 'repository' in job.keys():
            try:
                ...
                self.logger.info('failed writing pipeline log.')
        ...
        try:
            while True: ##run forever
                try:
                    ...
                    self.logger.info('Pipeline Manager: Keyboard interupt detected, asking user...')
            ...
            self.logger.error('Pipeline Manager: Exception caught: ' + str(type(Exception)) + ' ' + str(inst))
            raise inst

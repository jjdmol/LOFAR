...
void RCUProtocolWrite::sendrequest()
{
  ...
  rcuprotocol.hdr.set(hdr, 1 << (getCurrentIndex() / MEPHeader::N_POL), MEPHeader::WRITE,
		      sizeof(i2c_protocol));
  rcuprotocol.data.setBuffer(i2c_protocol, sizeof(i2c_protocol));

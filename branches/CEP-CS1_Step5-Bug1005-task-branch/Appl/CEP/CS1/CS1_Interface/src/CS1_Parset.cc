...
vector<uint32> CS1_Parset::station2TABGroups() const
{
  ...
  vector<uint32> tabGroups;
  bool found = false;
  vector<string> stNames = getStringVector("Observation.VirtualInstrument.stationList");
  
  tabGroups.resize(stNames.size());
  
  for (uint station = 0; station < stNames.size(); station++) {
    for (uint tab(1); !found && tab <= nrTABs(); tab++) {
      vector<string> tiedArrayStations = getStringVector(string(formatString("Observation.TAB[%d]",tab)));
      for (uint i(0); i < tiedArrayStations.size(); i++) {
        if (stNames[station] == tiedArrayStations[i] ) {
	  tabGroups[station] = tab;
	  found = true;
	  break;
	}
      }
    ...
    if (!found) {
      tabGroups[station] = station + nrTABs()+1;
    }
    found = false;
  }
  return tabGroups;

...
BeamFormer::BeamFormer(unsigned nrStations, unsigned nrSamplesPerIntegration, 
		       unsigned nrBeamFormedStations, std::vector<unsigned> &station2BeamFormedStation)
:
...
#if 0
  cerr << "Beam forming station " << beamFormedStation << ", size is " << nrStationsInBeam << " (";
...
# if 0
      cerr << "dropping station " << stationList[i] << ", " << filteredData->flags[destStation].count() <<
	" samples were flagged away, upper bound = " << upperBound << endl;
#endif
...
//  cerr << "total Stations in beam = " << nrStationsInBeam << ", valid = " << nrValidStations << endl; 

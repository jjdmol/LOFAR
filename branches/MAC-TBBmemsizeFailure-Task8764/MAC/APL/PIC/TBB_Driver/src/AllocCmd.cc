...
{
    switch (itsStage) {
        // stage 1, get board memory size in pages(2048 bytes)
        case 0: {
            ...
            if (event.signal == F_TIMER) {
                ...
                if (tp_ack.status != 0) {
                    ...
                    TS->setMemorySize(getBoardNr(), 0);
                } 
                else {
                    // max. 32 GByte board, 2048 bytes in one page -> nr pages = (32 * 1024 *1024 *1024) / 2048 = 16777216 pages  
                    if (tp_ack.npages > 16777216) {
                        TS->setMemorySize(getBoardNr(), 0);
        ...
        for (int mpnr = 0; mpnr < TS->nrMpsOnBoard(); mpnr++) {
            for (int chnr = 0; chnr < TS->nrChannelsOnMp(); chnr++) {
                int ch = (mpnr * TS->nrChannelsOnMp()) + chnr;
                    TS->setChStartAddr((ch + (boardnr * TS->nrChannelsOnBoard())), 0);
                    TS->setChPageSize( (ch + (boardnr * TS->nrChannelsOnBoard())), 0);
            }
        }
        return (false);
    }

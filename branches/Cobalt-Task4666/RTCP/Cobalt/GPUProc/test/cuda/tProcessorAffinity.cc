...
// #   pragma omp parallel for num_threads(2)
  ...
  // set flags allowing the discovery of IO
  ...
  hwloc_topology_load(topology);
    ...
    hwloc_cpuset_dup(hwloc_topology_get_complete_cpuset(topology));
  try {
    gpu::Platform pf;
    cout << "Detected " << pf.size() << " CUDA devices" << endl;
  } catch (gpu::CUDAException& e) {
  ...
  // Return the cpu set that is the closest to the current gpu device.
  hwloc_cuda_get_device_cpuset(topology, *device.getDevice(), cpuset);
  ...
  stringlen = hwloc_cpuset_asprintf(&a_string, cpuset);
  ...
  hwloc_obj_attr_snprintf(a_string, 100, obj, ",", 1);
  cout << "object info:" << a_string << endl;
  
  if (hwloc_obj_get_info_by_name(obj , "name") == NULL)
    cout << "danger robinson" << endl;

...
namespace LOFAR
{
  namespace Cobalt
  {
    ...
    const unsigned BeamletBuffer<SAMPLE_TYPE>::itsNrTimesPerPacket;
    ...
    template<typename SAMPLE_TYPE>
    BeamletBuffer<SAMPLE_TYPE>::BeamletBuffer(const Parset &ps, const std::string &stationName, unsigned rspBoard)
      :
    ...
    template<>
    inline void BeamletBuffer<i4complex>::writePacket(i4complex *dst, const i4complex *src)
    {
      _copy_pkt_to_bbuffer_32_bytes(dst, itsStride, src, itsNrSubbands);
    }

    template<>
    inline void BeamletBuffer<i8complex>::writePacket(i8complex *dst, const i8complex *src)
    {
      _copy_pkt_to_bbuffer_64_bytes(dst, itsStride, src, itsNrSubbands);
    }

    template<>
    inline void BeamletBuffer<i16complex>::writePacket(i16complex *dst, const i16complex *src)
    {
    ...
    template<typename SAMPLE_TYPE>
    inline void BeamletBuffer<SAMPLE_TYPE>::writePacket(SAMPLE_TYPE *dst, const SAMPLE_TYPE *src)
    {
    ...
    template<typename SAMPLE_TYPE>
    inline void BeamletBuffer<SAMPLE_TYPE>::updateValidData(const TimeStamp &begin, const TimeStamp &end)
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::writeConsecutivePackets(unsigned count)
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::resetCurrentTimeStamp(const TimeStamp &newTimeStamp)
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::writeMultiplePackets(const void *rspData, const std::vector<TimeStamp> &timeStamps)
    {
      itsWriteTimer.start();
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::writePacketData(const SAMPLE_TYPE *data, const TimeStamp &begin)
    {
      itsWriteTimer.start();
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::startReadTransaction(const std::vector<TimeStamp> &begin, unsigned nrElements)
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::sendSubband(Stream *str, unsigned subband, unsigned beam) const
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::sendUnalignedSubband(Stream *str, unsigned subband, unsigned beam) const
    {
    ...
    template<typename SAMPLE_TYPE>
    SparseSet<unsigned> BeamletBuffer<SAMPLE_TYPE>::readFlags(unsigned beam)
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::stopReadTransaction()
    {
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::noMoreReading()
    {
      if (!itsIsRealTime)
    ...
    template<typename SAMPLE_TYPE>
    void BeamletBuffer<SAMPLE_TYPE>::noMoreWriting()
    {
      if (!itsIsRealTime)

...
#if 0
TEST(ComplVolt_extensive) {
    // CV XXYY   : 2 TABs: 17 sb in 5 parts per TAB: 4 * 2 * 5 = 40 files
    parset.add("Observation.ObsID", "123882");
  parset.add("Cobalt.BeamFormer.CoherentStokes.timeIntegrationFactor", "1");
  parset.add("Cobalt.BeamFormer.CoherentStokes.nrChannelsPerSubband", "64");
  parset.add("Cobalt.BeamFormer.CoherentStokes.which", "XXYY"); // complex voltages


    Parset parset = commomKeys();
    parset.replace("Observation.DataProducts.Output_CoherentStokes.enabled", "true");
    parset.add("Observation.nrBeams", "2");
    parset.add("Observation.Beam[0].subbandList", "[100..116]"); // 17 sb in beam 0
    parset.add("Observation.Beam[0].nrTiedArrayBeams", "2");
    parset.add("Observation.Beam[1].subbandList", "[100..116]"); // 17 sb in beam 0
    parset.add("Observation.Beam[0].nrTiedArrayBeams", "3");
    parset.add("Observation.rspBoardList", "[16*0]");
    parset.add("Observation.rspSlotList", "[0..16]");
    parset.add("Cobalt.BeamFormer.CoherentStokes.subbandsPerFile", "4"); // last file will have 1 sb

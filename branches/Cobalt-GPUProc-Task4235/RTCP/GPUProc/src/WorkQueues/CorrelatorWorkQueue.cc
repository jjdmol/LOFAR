...
namespace LOFAR
{
  ...
  {     
    ...
    WorkQueue( context, device, gpuNumber, parset),
      ...
      devBufferInput(context, CL_MEM_READ_WRITE, ps.nrStations() * NR_POLARIZATIONS * ps.nrSamplesPerSubband() * sizeof(std::complex<float>)),
      ...
      inputSamples(boost::extents[ps.nrStations()][(ps.nrSamplesPerChannel() + NR_TAPS - 1) * ps.nrChannelsPerSubband()][NR_POLARIZATIONS][ps.nrBytesPerComplexSample()], 
      queue, CL_MEM_WRITE_ONLY, devBufferInput),
      devFilteredData(devBufferB),
      devCorrectedData(devBufferInput),

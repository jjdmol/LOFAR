...
class smartdppp(LOFARnodeTCP):
    ...
    def run(self, infile, outfile, parmdb, ateam_sourcedb, estimate_sourcedb, target_sourcedb,
        ...
        self.logger.debug("ateam_sourcedb  = %s" % ateam_sourcedb)
        self.logger.debug("estimate_sourcedb = %s" % estimate_sourcedb)
        self.logger.debug("target_sourcedb = %s" % target_sourcedb)
        ...
        with log_time(self.logger):
            ...
            kwargs = {
                ...
                'tmpfile' : tmpfile,
                'parmdb' : parmdb,
                'ateam_sourcedb' : ateam_sourcedb,
                'estimate_sourcedb' : estimate_sourcedb,
                'target_sourcedb' : target_sourcedb,
    ...
    def _prepare_steps(self, **kwargs):
        ...
        for step in parset.getStringVector('steps'):
            if parset.getString(step + '.type', '').startswith('smartdemix'):
                try:
                    parset.getStringVector(step + '.sources')
                except:
                    parset.add(step + '.sources', [])
    ...
    def _prepare_demix_step(self, stepname, **kwargs):
        ...
        if kwargs['ateam_sourcedb']:
            patch_dictionary[stepname + '.ateam.skymodel'] = kwargs['ateam_sourcedb']
        if kwargs['estimate_sourcedb']:
            patch_dictionary[stepname + '.estimate.skymodel'] = kwargs['estimate_sourcedb']
        if kwargs['target_sourcedb']:
            patch_dictionary[stepname + '.target.skymodel'] = kwargs['target_sourcedb']
...
if __name__ == "__main__":
    ...
    sys.exit(smartdppp(jobid, jobhost, jobport).run_with_stored_arguments())

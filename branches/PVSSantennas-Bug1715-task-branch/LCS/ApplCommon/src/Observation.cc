	...
	splitterOn(false)
{
	...
	string CNprefix = aParSet->locateModule("CNProc");
	if (!CNprefix.empty()) {
	    ...
	    std::vector<unsigned> phaseTwoPsets = aParSet->getUint32Vector(CNprefix+"CNProc.phaseTwoPsets" );
	    if (phaseTwoPsets.empty()) 
	      for (unsigned p = 0; p < 64; p++)
		phaseTwoPsets.push_back(p);

	    // by default, use all psets
	    std::vector<unsigned> phaseThreePsets = aParSet->getUint32Vector(CNprefix+"CNProc.phaseThreePsets" );
	    if (phaseThreePsets.empty()) 
	      for (unsigned p = 0; p < 64; p++)
		phaseThreePsets.push_back(p);

	    for (size_t d = 0; d < nrDataProducts; d ++) {
	      bool enabled = aParSet->getBool(prefix+str(format("DataProducts.Output_%s.enabled") % dataProductNames[d]));

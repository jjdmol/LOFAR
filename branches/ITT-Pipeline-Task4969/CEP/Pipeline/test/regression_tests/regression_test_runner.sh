...
then
  ...
  if [ -f /data/scratch/lofarbuild/testdata/regression_test_runner/${PIPELINE}/${PIPELINE}.parset ]
  then
    :
  else
    echo "*** Error ***"
    echo $"The selected pipeline does not exist as entry in the test data dir:"
    echo $"/data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE"
    exit 1
  fi
else
    echo "*** Error ***"
...
SECONDHOST=false
if [ -d $"/data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE/input_data/host2" ];
then
...
ssh localhost $"mkdir $WORKING_DIR -p"
ssh localhost $"rm $WORKING_DIR/* -rf"
...
scp -r $"/data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE/input_data/host1/"* $HOST1:$"$WORKING_DIR/input_data"
if [ $SECONDHOST == true ]
then
  ssh $HOST2 $"mkdir $WORKING_DIR/input_data"
  scp -r $"/data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE/input_data/host2/"* $HOST2:$"$WORKING_DIR/input_data"
fi
...
cp /data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE/$PIPELINE.parset $"$WORKING_DIR/$PIPELINE.parset" ||  { echo "The pipeline returned a non zero exit value, aborting test" ; exit 1; }
...
cp -r /data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE/target_data/host1/*  $WORKING_DIR/target_data/host1
if [ $SECONDHOST == true ]
then
  mkdir $WORKING_DIR/target_data/host2 -p
  cp -r /data/scratch/lofarbuild/testdata/regression_test_runner/$PIPELINE/target_data/host2/*  $WORKING_DIR/target_data/host2 || echo "*** Skipping comparison of node 2 ***"
fi

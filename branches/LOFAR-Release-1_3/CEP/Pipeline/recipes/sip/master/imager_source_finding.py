...
class imager_source_finding(BaseRecipe, RemoteCommandRecipeMixIn):
    ...
    def go(self):
        ...
        for host, data in input_map:
            ...
            created_sourcedbs.append((
                                host, self.inputs['sourcedb_target_path']))
        ...
        store_data_map(self.inputs['mapfile'], catalog_output_path_from_nodes)
        self.logger.debug("Wrote datamap with created sourcelists: {0}".format(
                                                self.inputs['mapfile']))

        store_data_map(self.inputs['sourcedb_map_path'], source_dbs_from_nodes)

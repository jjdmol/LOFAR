...
GCFEvent::TResult NenuFarIO::connect2server(GCFEvent& event, GCFPortInterface& port)
{
    ...
    case F_INIT:
		...
		try {
			...
			itsIOtimeout 	  = globalParameterSet()->getInt   ("BeamServer.NenuFar.respondTime", 3);
		}
		catch (Exception&	ex) {
			cerr << ex << endl;
			abort();
...
GCFEvent::TResult NenuFarIO::monitorAdmin(GCFEvent& event, GCFPortInterface& port)
{
	...
	case F_TIMER: {
		...
		if (!itsCurrentBeam.name().empty()) {		// still work to do?
			...
			itsTimerPort->setTimer(5.0);		// re-evaluate after 5 seconds.
		}
	} break;
...
GCFEvent::TResult NenuFarIO::tranceiveAdminChange(GCFEvent& event, GCFPortInterface& port)
{
	...
	case F_ENTRY: {	
		...
		default:
			LOG_ERROR_STR("Unsupported beam state (" << itsCurrentBeam.beamState() << "} for beam " << itsCurrentBeam.name());
			TRAN(NenuFarIO::monitorAdmin);
			return (GCFEvent::HANDLED);
		}
		...
		ParameterSet	answer = NenuFarMsg::unpack2parset(databuf, dataLen);
		// TODO: test if beamName matches name of msg we sent
		if (answer.isDefined("result")) {
			int		resultValue = answer.getInt("result");
			if (resultValue == 0) {
				itsBeams->setCommState(itsCurrentBeam.name(), itsCurrentBeam.beamState());
				LOG_INFO_STR("NenuFar acknowledged that beam " << itsCurrentBeam.name() << " is in state " << 
								itsCurrentBeam.beamState());
				TRAN(NenuFarIO::monitorAdmin);
				return (GCFEvent::HANDLED);
			}
		}
		LOG_ERROR_STR("NenuFar answer on beam " << itsCurrentBeam.name() << " doesn't contain a result-field");
		TRAN(NenuFarIO::monitorAdmin);
	} break;

	case F_DATAIN:
#if 0
		itsCurrentBeam.comm_state = ...
#endif
		break;

	case F_TIMER: {
		// too bad, no answer from the NCU continue
//		LOG_WARN_STR("Didn't receive an answer from NenuFar on command " << command);
		itsIOerror = true;
	} break;

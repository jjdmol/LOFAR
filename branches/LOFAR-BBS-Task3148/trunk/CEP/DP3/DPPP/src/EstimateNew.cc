...
public:
    ...
    cursor(T *pointer)
        :   m_pointer(pointer),
            m_rank(1)
    {
        std::fill(m_strides, m_strides + MAX_RANK, 0);
        m_strides[0] = 1;
        m_stride = m_strides[0];
    }
    ...
    cursor(T *pointer, size_t rank, const T_STRIDE *strides)
        :   m_pointer(pointer),
            m_rank(rank)
    {
        std::copy(strides, strides + m_rank, m_strides);
        std::fill(m_strides + m_rank, m_strides + MAX_RANK, 0);
        m_stride = m_strides[0];
    }
    ...
    {
        m_pointer += m_strides[i];
    }

    void forward(size_t i, size_t n)
    {
        m_pointer += n * m_strides[i];
    }
    ...
    const size_t &stride(size_t i) const
    {
        return m_strides[i];
    }

private:
    friend class const_cursor<T>;
    T*      m_pointer;
    size_t  m_rank;
    size_t  m_stride;
    size_t  m_strides[MAX_RANK];
};
...
public:
    ...
    {
        std::copy(other.m_strides, other.m_strides + MAX_RANK, m_strides);
    }
